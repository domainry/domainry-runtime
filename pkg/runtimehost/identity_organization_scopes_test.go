...
func TestRuntimeOrganizationScopeResolverBridgesPartyFacts(t *testing.T) {
	resolver := runtimeOrganizationScopeResolver{resolve: func(_ context.Context, workspaceID string, workforceProfileIDs []string) (partymodel.OrganizationScopeFacts, error) {
	...
	facts, err := resolver.ResolveIdentityOrganizationScopes(t.Context(), "workspace", []string{"workforce-1"})
