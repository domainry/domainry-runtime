...
func (a *Runtime) close(ctx context.Context) error {
	...
	if a.borrowedStore {
		return errors.Join(releaseErr, notificationErr, monitoringErr, partyErr)
	}
	return errors.Join(releaseErr, notificationErr, monitoringErr, partyErr, a.store.Close())
}
