...
func TestSchedulerApplicationDefinitionSourceUsesMetadataBusinessPort(t *testing.T) {
	empty := schedulerApplicationDefinitionSource{}
	...
	if _, err := (schedulerApplicationDefinitionSource{definitions: schedulerMetadataDefinitions{err: wantErr}}).ListSchedulerDefinitions(t.Context()); !errors.Is(err, wantErr) {
	...
	source := schedulerApplicationDefinitionSource{definitions: schedulerMetadataDefinitions{values: []metadatasdk.Definition{valid}}}
	...
	if _, err := (schedulerApplicationDefinitionSource{definitions: schedulerMetadataDefinitions{values: []metadatasdk.Definition{{ResourceKey: "bad", Payload: []byte("{")}}}}).ListSchedulerDefinitions(t.Context()); err == nil {
...
func TestSchedulerWiringSmallHelpers(t *testing.T) {
	...
	if service := newSchedulerApplicationService(nil, workerplatform.Dependencies{}); service == nil {
