...
func TestSchedulerSDKDefinitionPreservesCalendarAndRoutesModuleHTTPThroughRuntime(t *testing.T) {
	definition := schedulerSDKDefinition(recordmodel.Record{ID: "partner_sync", UpdatedAt: "2026-08-29T12:00:00Z", Data: map[string]any{
...
func TestSchedulerSDKDefinitionMapsRuntimeOwnerWithoutEmbeddingBusinessLogic(t *testing.T) {
	definition := schedulerSDKDefinition(recordmodel.Record{ID: "daily", Data: map[string]any{"status": "enabled", "schedule_type": "interval", "interval_seconds": 60, "target_type": "workflow", "target_key": "scheduled:daily", "payload_json": `{}`}})
