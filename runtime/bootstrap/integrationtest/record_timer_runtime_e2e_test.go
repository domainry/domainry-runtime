...
func (s recordTimerWorkspaceSchema) SchemaForPrincipal(context.Context, principalmodel.Principal) appschemamodel.ApplicationSchemaSnapshot {
	return s.snapshot
}
...
func (r *recordTimerWorkspaceRuntime) ExecuteRecordTimer(_ context.Context, execution recordtimerapplication.RecordTimerExecution, _ principalmodel.Principal) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func TestRecordTimerCommitsAtomicallyAndClaimsInStableOrderWithFencing(t *testing.T) {
	store := openRecordTimerRuntimeTestStore(t)
	defer store.Close()
	objects := append(recordTimerRuntimeTestObjects(), definitionmodel.ObjectSchema{Key: "service_request", Fields: []definitionmodel.FieldSchema{{Key: "status", Type: "select"}}})
	if err := metadataStore(store).SyncManifest(t.Context(), recordTimerInstallationScope(), manifestmodel.ManifestSchema{Objects: objects}); err != nil {
		t.Fatal(err)
	}
	service := newRecordTimerRuntimeTestService(t, store, objects).Applications().RecordTimers
	service.ConfigureWorker(recordtimerapplication.WorkerConfig{Enabled: true, LeaseTTL: time.Minute, BatchSize: 25})
	...
	timerCommit, err := service.BuildRecordTimerMutation(t.Context(), "workspace-primary", recordtimerapplication.RecordTimerSchedule{
	...
	supersede, err := service.BuildRecordTimerSupersedeMutations(t.Context(), "workspace-primary", timerCommit.Record, recordtimerapplication.RecordTimerSchedule{
	...
	cancelled, err := service.CancelRecordTimers(t.Context(), "workspace-primary", source.Key, sourceRecord.ID, "response", now.Add(2*time.Second), recordTimerRuntimeSystemScope())
	...
	for _, request := range []recordtimerapplication.RecordTimerSchedule{
	...
	leases, err := service.ClaimDueRecordTimers(t.Context(), "workspace-primary", now, 3, recordTimerRuntimeSystemScope())
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := service.FinishRecordTimer(t.Context(), "workspace-primary", leases[0], now.Add(time.Second), recordTimerRuntimeSystemScope()); err != nil {
		t.Fatal(err)
	}

	reclaimed, err := newRecordTimerRuntimeTestService(t, store, objects).Applications().RecordTimers.ClaimDueRecordTimers(t.Context(), "workspace-primary", now.Add(2*time.Minute), 1, recordTimerRuntimeSystemScope())
	if err != nil || len(reclaimed) != 1 {
		t.Fatalf("expired timer reclaim = %#v err=%v", reclaimed, err)
	}
	if err := service.FinishRecordTimer(t.Context(), "workspace-primary", leases[1], now.Add(2*time.Minute), recordTimerRuntimeSystemScope()); !mutation.IsMutationConflict(err, mutation.MutationConflictLeaseLost) {
		t.Fatalf("stale timer completion error = %v", err)
	}
	if err := service.FinishRecordTimer(t.Context(), "workspace-primary", reclaimed[0], now.Add(2*time.Minute), recordTimerRuntimeSystemScope()); err != nil {
...
func TestRecordTimerConcurrentWorkersClaimOneTimerOnce(t *testing.T) {
	store := openRecordTimerRuntimeTestStore(t)
	defer store.Close()
	objects := recordTimerRuntimeTestObjects()
	if err := metadataStore(store).SyncManifest(t.Context(), recordTimerInstallationScope(), manifestmodel.ManifestSchema{Objects: objects}); err != nil {
		t.Fatal(err)
	}
	now := time.Date(2026, 7, 21, 8, 0, 0, 0, time.UTC)
	seed := newRecordTimerRuntimeTestService(t, store, objects).Applications().RecordTimers
	commit, err := seed.BuildRecordTimerMutation(t.Context(), "workspace-primary", recordtimerapplication.RecordTimerSchedule{TimerKey: "once", ObjectKey: "record", RecordID: "one", Purpose: "fire", DueAt: now, TargetType: "action", TargetKey: "record.fire"}, now)
	if err != nil {
		t.Fatal(err)
	}
	if err := recordLegacyStore(store).CommitRecordMutationBatch(t.Context(), "workspace-primary", []transactionmodel.RecordMutationCommit{commit}); err != nil {
	...
	for range 100 {
		...
		go func() {
			...
			<-start
			service := newRecordTimerRuntimeTestService(t, store, objects).Applications().RecordTimers
			leases, claimErr := service.ClaimDueRecordTimers(t.Context(), "workspace-primary", now, 1, recordTimerRuntimeSystemScope())
			if claimErr != nil {
...
func TestRecordTimerCancellationDrainsEveryPage(t *testing.T) {
	...
	for index := range 501 {
		commit, err := service.BuildRecordTimerMutation(t.Context(), "workspace-primary", recordtimerapplication.RecordTimerSchedule{
	...
	cancelled, err := service.CancelRecordTimers(t.Context(), "workspace-primary", "source", "source-1", "future", now, recordTimerRuntimeSystemScope())
...
func TestRecordTimerFailureReleasesEntireClaimBatchAndStopsAtAttemptLimit(t *testing.T) {
	...
	for index, maxAttempts := range []int{1, 2} {
		if _, err := service.Schedule(t.Context(), "workspace-primary", recordtimerapplication.RecordTimerSchedule{
		...
		}, recordmodel.Record{}, nil, now, recordTimerRuntimeSystemScope()); err != nil {
			t.Fatal(err)
		}
	}
	if processed, err := service.ProcessDueRecordTimers(t.Context(), "workspace-primary", now, 10, recordTimerRuntimeSystemScope()); err == nil || processed != 0 {
	...
	if processed, err := service.ProcessDueRecordTimers(t.Context(), "workspace-primary", now.Add(2*time.Second), 10, recordTimerRuntimeSystemScope()); err == nil || processed != 0 {
...
func TestRecordTimerWorkerProcessesEveryWorkspace(t *testing.T) {
	...
	service := recordtimerapplication.NewRecordTimerApplicationServiceWithWorker(
		recordTimerWorkspaceSchema{snapshot: appschemamodel.ApplicationSchemaSnapshot{Objects: objects}}, runtime, recordLegacyStore(store),
	...
	for _, workspaceID := range []string{"tenant-a", "tenant-b"} {
		if _, err := service.Schedule(t.Context(), workspaceID, recordtimerapplication.RecordTimerSchedule{
	...
	processed, err := service.ProcessDueForAllWorkspaces(t.Context(), now, 10, recordTimerRuntimeSystemScope())
...
func TestRecordTimerWorkspaceFailureDoesNotStarveOtherTenantOrExceedGlobalBatch(t *testing.T) {
	...
	processed, err := service.ProcessDueForAllWorkspaces(t.Context(), now, 3, recordTimerRuntimeSystemScope())
