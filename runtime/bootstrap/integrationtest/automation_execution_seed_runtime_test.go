...
func TestAutomationRuleEditSurvivesRuntimeRestart(t *testing.T) {
	dir := t.TempDir()
	cfg := config.Config{
		AppLocale: "en-US", DatabaseDriver: "sqlite", DBPath: filepath.Join(dir, "runtime.db"),
		ManifestPath: filepath.Join("..", "..", "domain", "manifest", "testdata", "manifests", "crm-customer-360.json"), UploadDir: filepath.Join(dir, "uploads"),
	}
	const ruleKey = "customer.verify_business_license"
	...
	if len(reloaded.Instructions) != len(rule.Instructions) {
		t.Fatalf("expected canonical instruction order to survive restart, got %#v", reloaded.Instructions)
	}
	for index := range rule.Instructions {
		if reloaded.Instructions[index].Key != rule.Instructions[index].Key {
			t.Fatalf("Instruction order changed at %d: expected %s, got %s", index, rule.Instructions[index].Key, reloaded.Instructions[index].Key)
		}
	}
	position, ok := reloaded.Layout.Nodes["ocr"]
	if !ok || position.X != 420 || position.Y != 180 {
		t.Fatalf("expected persisted canvas layout after restart, got %#v", reloaded.Layout)
	}
}
