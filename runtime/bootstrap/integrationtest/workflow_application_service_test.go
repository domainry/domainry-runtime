...
func TestWorkflowApplicationServiceAtomicallyRejectsTerminalApproval(t *testing.T) {
	...
	role := accessfixture.Bundle{Key: "manager", Permissions: []string{"workflow.task.act"}, RecordScope: "all_records"}
	records := runtimetestkit.NewRuntimeServices(t.Context(), runtimetestkit.RuntimeServicesConfig{TemplateID: "workflow-decision", TemplateVersion: "1", Name: "Workflow Decision", Objects: nil, Views: nil, Actions: nil, Workflows: []definitionmodel.WorkflowSchema{workflow}, AutomationRules: nil, Dictionaries: nil, Integrations: integrationmodel.IntegrationSchema{}, Reports: nil, Entrypoints: nil, Skills: nil, Agents: nil, Store: store, WorkflowProcesses: workflowpersistence.NewWorkflowProcessStore(store), WorkflowDecisions: workflowpersistence.NewWorkflowDecisionStore(store), WorkflowWorker: workflowpersistence.NewWorkflowWorkerStore(store)})
	...
	for _, insert := range []func() error{
		func() error { return workflowProcessStore(store).InsertProcess(t.Context(), "default", process) },
		func() error { return workflowProcessStore(store).InsertNode(t.Context(), "default", node) },
		func() error { return workflowProcessStore(store).InsertTask(t.Context(), "default", task) },
		func() error {
			return workflowWorkerStore(store).InsertExecution(t.Context(), "default", execution)
		},
	} {
	...
	result, err := records.Applications().Workflows.DecideTask(t.Context(), task.ID, workflowmodel.WorkflowTaskDecisionRequest{Decision: "rejected", Comment: "Not approved"}, accessfixture.Attach(principalmodel.Principal{Principal: identitysdk.Principal{Known: true, UserID: "manager", WorkspaceID: "default"}}, role))
	if err != nil {
	...
	persistedExecution, ok, err := workflowWorkerStore(store).GetExecution(t.Context(), "default", process.ID)
	...
	events, err := workflowProcessStore(store).ListEvents(t.Context(), "default", process.ID, 20)
...
func TestWorkflowDecisionQueuesAndWorkerResumesGraphContinuation(t *testing.T) {
	...
	role := accessfixture.Bundle{Key: "manager", Permissions: []string{"workflow.task.act"}, RecordScope: "all_records"}
	records := runtimetestkit.NewRuntimeServices(t.Context(), runtimetestkit.RuntimeServicesConfig{TemplateID: "workflow-continuation", TemplateVersion: "1", Name: "Workflow Continuation", Objects: nil, Views: nil, Actions: nil, Workflows: []definitionmodel.WorkflowSchema{workflow}, AutomationRules: nil, Dictionaries: nil, Integrations: integrationmodel.IntegrationSchema{}, Reports: nil, Entrypoints: nil, Skills: nil, Agents: nil, Store: store, WorkflowProcesses: workflowpersistence.NewWorkflowProcessStore(store), WorkflowDecisions: workflowpersistence.NewWorkflowDecisionStore(store), WorkflowWorker: workflowpersistence.NewWorkflowWorkerStore(store)})
	...
	for _, insert := range []func() error{func() error { return workflowProcessStore(store).InsertProcess(t.Context(), "default", process) }, func() error { return workflowProcessStore(store).InsertNode(t.Context(), "default", node) }, func() error { return workflowProcessStore(store).InsertTask(t.Context(), "default", task) }} {
		if err := insert(); err != nil {
			t.Fatal(err)
		}
	}
	completed, err := records.Applications().Workflows.DecideTask(t.Context(), task.ID, workflowmodel.WorkflowTaskDecisionRequest{Decision: "approved"}, accessfixture.Attach(principalmodel.Principal{Principal: identitysdk.Principal{Known: true, UserID: "manager", WorkspaceID: "default"}}, role))
	if err != nil || completed.Status != "completed" {
		t.Fatalf("expected request-owned continuation to complete, process=%#v err=%v", completed, err)
	}
	intent, ok, err := workflowWorkerStore(store).GetExecution(t.Context(), "default", process.ID)
...
func TestWorkflowDecisionCreatesNextApprovalTasksInSameCommit(t *testing.T) {
	...
	records := runtimetestkit.NewRuntimeServices(t.Context(), runtimetestkit.RuntimeServicesConfig{TemplateID: "two-approvals", TemplateVersion: "1", Name: "Two Approvals", Objects: nil, Views: nil, Actions: nil, Workflows: []definitionmodel.WorkflowSchema{workflow}, AutomationRules: nil, Dictionaries: nil, Integrations: integrationmodel.IntegrationSchema{}, Reports: nil, Entrypoints: nil, Skills: nil, Agents: nil, Store: store, WorkflowProcesses: workflowpersistence.NewWorkflowProcessStore(store), WorkflowDecisions: workflowpersistence.NewWorkflowDecisionStore(store), WorkflowWorker: workflowpersistence.NewWorkflowWorkerStore(store), IdentityDirectory: identity})
	...
	result, err := records.Applications().Workflows.DecideTask(t.Context(), task.ID, workflowmodel.WorkflowTaskDecisionRequest{Decision: "approved"}, accessfixture.Attach(principalmodel.Principal{Principal: identitysdk.Principal{Known: true, UserID: "manager_1", WorkspaceID: "default"}}, role))
	...
	tasks, err := workflowProcessStore(store).ListTasks(t.Context(), "default", process.ID, "manager_2", "open", 10)
