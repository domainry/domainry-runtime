...
func TestWorkflowApplicationServiceAtomicallyRejectsTerminalApproval(t *testing.T) {
	...
	role := accessfixture.Bundle{Key: "manager", Permissions: integrationWorkflowTaskDecisionPermissions(), RecordScope: "all_records"}
	records := runtimetestkit.NewRuntimeServices(t.Context(), runtimetestkit.RuntimeServicesConfig{TemplateID: "workflow-decision", TemplateVersion: "1", Name: "Workflow Decision", Objects: nil, Actions: nil, Workflows: []definitionmodel.WorkflowSchema{workflow}, AutomationRules: nil, Dictionaries: nil, Integrations: connectormodel.IntegrationSchema{}, Reports: nil, Skills: nil, Agents: nil, Store: store, WorkflowProcesses: workflowpersistence.NewWorkflowProcessStore(store), WorkflowDecisions: workflowpersistence.NewWorkflowDecisionStore(store), WorkflowWorker: workflowpersistence.NewWorkflowWorkerStore(store)})
...
func TestWorkflowDecisionQueuesAndWorkerResumesGraphContinuation(t *testing.T) {
	...
	role := accessfixture.Bundle{Key: "manager", Permissions: integrationWorkflowTaskDecisionPermissions(), RecordScope: "all_records"}
	records := runtimetestkit.NewRuntimeServices(t.Context(), runtimetestkit.RuntimeServicesConfig{TemplateID: "workflow-continuation", TemplateVersion: "1", Name: "Workflow Continuation", Objects: nil, Actions: nil, Workflows: []definitionmodel.WorkflowSchema{workflow}, AutomationRules: nil, Dictionaries: nil, Integrations: connectormodel.IntegrationSchema{}, Reports: nil, Skills: nil, Agents: nil, Store: store, WorkflowProcesses: workflowpersistence.NewWorkflowProcessStore(store), WorkflowDecisions: workflowpersistence.NewWorkflowDecisionStore(store), WorkflowWorker: workflowpersistence.NewWorkflowWorkerStore(store)})
...
func TestWorkflowDecisionCreatesNextApprovalTasksInSameCommit(t *testing.T) {
	...
	records := runtimetestkit.NewRuntimeServices(t.Context(), runtimetestkit.RuntimeServicesConfig{TemplateID: "two-approvals", TemplateVersion: "1", Name: "Two Approvals", Objects: nil, Actions: nil, Workflows: []definitionmodel.WorkflowSchema{workflow}, AutomationRules: nil, Dictionaries: nil, Integrations: connectormodel.IntegrationSchema{}, Reports: nil, Skills: nil, Agents: nil, Store: store, WorkflowProcesses: workflowpersistence.NewWorkflowProcessStore(store), WorkflowDecisions: workflowpersistence.NewWorkflowDecisionStore(store), WorkflowWorker: workflowpersistence.NewWorkflowWorkerStore(store), IdentityDirectory: identity})
