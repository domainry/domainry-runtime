...
func TestRuntimeCRMLifecycleRoleMatrixAndExceptionPaths(t *testing.T) {
	...
	runtimeRoleFixtureRequestStatus(t, handler, "sales_rep", http.MethodPost, "/records/objects/lead/records/"+managerLeadID+"/actions/lead.qualify", map[string]any{"data": map[string]any{}}, http.StatusForbidden)
	runtimeRoleFixtureRequestStatus(t, handler, "sales_rep", http.MethodPost, "/records/objects/lead/records/"+repLeadID+"/actions/lead.convert", map[string]any{"data": map[string]any{}}, http.StatusBadRequest)
	repLead := runtimeRoleFixtureRequest[map[string]any](t, handler, "sales_rep", http.MethodPost, "/records/objects/lead/records/"+repLeadID+"/actions/lead.qualify", map[string]any{"data": map[string]any{}})
	assertRuntimeFixtureRecordField(t, repLead, "status", "qualified")
	repLead = runtimeRoleFixtureRequest[map[string]any](t, handler, "sales_rep", http.MethodPost, "/records/objects/lead/records/"+repLeadID+"/actions/lead.convert", map[string]any{"data": map[string]any{}})
	...
	runtimeRoleFixtureRequestStatus(t, handler, "sales_rep", http.MethodPost, "/records/objects/opportunity/records/"+repOpportunityID+"/actions/opportunity.mark_won", map[string]any{"data": map[string]any{}}, http.StatusBadRequest)
	repOpportunity := runtimeRoleFixtureRequest[map[string]any](t, handler, "sales_rep", http.MethodPost, "/records/objects/opportunity/records/"+repOpportunityID+"/actions/opportunity.advance_stage", map[string]any{"data": map[string]any{"stage": "negotiation"}})
	assertRuntimeFixtureRecordField(t, repOpportunity, "stage", "negotiation")
	repOpportunity = runtimeRoleFixtureRequest[map[string]any](t, handler, "sales_rep", http.MethodPost, "/records/objects/opportunity/records/"+repOpportunityID+"/actions/opportunity.mark_won", map[string]any{"data": map[string]any{}})
	...
	repActivity := runtimeRoleFixtureRequest[map[string]any](t, handler, "sales_rep", http.MethodPost, "/records/objects/activity/records/"+repActivityID+"/actions/activity.complete", map[string]any{"data": map[string]any{}})
	assertRuntimeFixtureRecordField(t, repActivity, "status", "completed")
	runtimeRoleFixtureRequestStatus(t, handler, "sales_rep", http.MethodPost, "/records/objects/activity/records/"+repActivityID+"/actions/activity.start", map[string]any{"data": map[string]any{}}, http.StatusBadRequest)

	draftContractID := runtimeRoleFixtureRecordIDByField(t, handler, "sales_manager", "contract", "status", "draft")
	runtimeRoleFixtureRequestStatus(t, handler, "finance_reviewer", http.MethodPost, "/records/objects/contract/records/"+draftContractID+"/actions/contract.sign", map[string]any{"data": map[string]any{}}, http.StatusForbidden)
	duePaymentID := runtimeRoleFixtureRecordIDByField(t, handler, "finance_reviewer", "payment", "status", "due")
	runtimeRoleFixtureRequestStatus(t, handler, "sales_rep", http.MethodPost, "/records/objects/payment/records/"+duePaymentID+"/actions/payment.mark_collected", map[string]any{"data": map[string]any{}}, http.StatusForbidden)
	financePayment := runtimeRoleFixtureRequest[map[string]any](t, handler, "finance_reviewer", http.MethodPost, "/records/objects/payment/records/"+duePaymentID+"/actions/payment.mark_collected", map[string]any{"data": map[string]any{}})
...
func runtimeRoleFixtureRecordIDByField(t *testing.T, handler http.Handler, role string, objectKey string, field string, expected any) string {
	t.Helper()
	page := runtimeRoleFixtureRequest[map[string]any](t, handler, role, http.MethodGet, "/records/objects/"+objectKey+"/records?page=1&page_size=50", nil)
