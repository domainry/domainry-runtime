...
func TestRuntimeCRMReportObjectExportsRetireTheUngovernedSecondQuery(t *testing.T) {
	...
	for _, path := range []string{
		"/report/crm_pipeline_health/exports/opportunity",
		"/report/crm_revenue_collection/exports/payment",
	} {
	...
	runtimeFixtureRawRequest(t, handler, "sales_manager", http.MethodGet, "/report/crm_pipeline_health/summary?page_size=50", nil, http.StatusOK)
	runtimeFixtureRawRequest(t, handler, "finance_reviewer", http.MethodGet, "/report/crm_revenue_collection/summary?page_size=50", nil, http.StatusOK)
	runtimeFixtureRawRequest(t, handler, "sales_rep", http.MethodGet, "/report/crm_revenue_collection/summary?page_size=50", nil, http.StatusForbidden)
}
