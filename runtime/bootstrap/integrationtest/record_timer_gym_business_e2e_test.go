...
func (r *gymTimerFixtureRuntime) ExecuteRecordTimer(ctx context.Context, execution recordtimerapplication.RecordTimerExecution, _ principalmodel.Principal) error {
...
func gymTimerFixtureObjects() []definitionmodel.ObjectSchema {
	objects := append([]definitionmodel.ObjectSchema(nil), recordtimerprojection.RecordTimerSystemObjects()...)
...
func gymTimerFixtureService(t *testing.T) (*recordtimerapplication.RecordTimerApplicationService, recordpersistence.RecordStore, *gymTimerFixtureRuntime, []definitionmodel.ObjectSchema) {
	t.Helper()
	...
	service := recordtimerapplication.NewRecordTimerApplicationServiceWithWorker(gymTimerFixtureSchema{snapshot: appschemamodel.ApplicationSchemaSnapshot{Objects: objects}}, runtime, repository, workerplatform.Dependencies{WorkerID: workerplatform.WorkerID("gym-timer-fixture")})
	service.ConfigureWorker(recordtimerapplication.WorkerConfig{Enabled: true, BatchSize: 100, LeaseTTL: time.Minute})
...
func gymTimerSchedule(t *testing.T, service *recordtimerapplication.RecordTimerApplicationService, now time.Time, request recordtimerapplication.RecordTimerSchedule) recordmodel.Record {
	t.Helper()
	timer, err := service.Schedule(t.Context(), "workspace-primary", request, recordmodel.Record{ID: request.RecordID}, nil, now, recordTimerRuntimeSystemScope())
	if err != nil {
		t.Fatal(err)
	}
	return timer
...
func TestGymWaitlistTimeoutAdvancesOnlyNextEntryAndRejectsLateConfirmation(t *testing.T) {
	...
	waitlist := recordTimerRuntimeObjectByKey(t, objects, "gym_waitlist_entry")
	...
	gymTimerSchedule(t, service, now, recordtimerapplication.RecordTimerSchedule{TimerKey: "wait-1-timeout", ObjectKey: waitlist.Key, RecordID: "wait-1", Purpose: "offer_timeout", DueAt: now, TargetType: "action", TargetKey: "waitlist.offer_timeout", Sequence: 1})
	processed, err := service.ProcessDueRecordTimers(t.Context(), "workspace-primary", now, 10, recordTimerRuntimeSystemScope())
...
func TestGymClassCancellationCancelsEveryFutureAttendanceTimer(t *testing.T) {
	service, repository, _, objects := gymTimerFixtureService(t)
	now := time.Date(2026, 7, 21, 12, 0, 0, 0, time.UTC)
	classSession := recordTimerRuntimeObjectByKey(t, objects, "gym_class_session")
	...
	for index, purpose := range []string{"check_in_open", "check_in_close", "no_show", "attendance_finalize"} {
		gymTimerSchedule(t, service, now, recordtimerapplication.RecordTimerSchedule{TimerKey: purpose, ObjectKey: classSession.Key, RecordID: "class-1", Purpose: purpose, DueAt: now.Add(time.Duration(index+1) * time.Hour), TargetType: "action", TargetKey: "attendance.window", Sequence: int64(index)})
	}
	cancelled, err := service.CancelRecordTimers(t.Context(), "workspace-primary", classSession.Key, "class-1", "", now, recordTimerRuntimeSystemScope())
...
func TestGymUrgentTicketResponseCancelsAlertAndUnansweredEscalatesOnce(t *testing.T) {
	...
	now := time.Date(2026, 7, 21, 12, 0, 0, 0, time.UTC)
	tickets := recordTimerRuntimeObjectByKey(t, objects, "gym_service_ticket")
	for _, id := range []string{"ticket-responded", "ticket-unanswered"} {
		...
		gymTimerSchedule(t, service, now, recordtimerapplication.RecordTimerSchedule{TimerKey: id + ":sla", ObjectKey: tickets.Key, RecordID: id, Purpose: "response_sla", DueAt: now.Add(2 * time.Hour), TargetType: "action", TargetKey: "ticket.escalate"})
	}
	...
	if cancelled, err := service.CancelRecordTimers(t.Context(), "workspace-primary", tickets.Key, "ticket-responded", "response_sla", now.Add(time.Hour), recordTimerRuntimeSystemScope()); err != nil || cancelled != 1 {
		t.Fatalf("cancel responded ticket timer=%d err=%v", cancelled, err)
	}
	for range 2 {
		if _, err := service.ProcessDueRecordTimers(t.Context(), "workspace-primary", now.Add(3*time.Hour), 10, recordTimerRuntimeSystemScope()); err != nil {
			t.Fatal(err)
		}
	}
	runtime.mu.Lock()
...
func TestGymCardPackageExpiryAndNinetyDayReviewUseRelativeRecordTimers(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		timer, err := service.Schedule(t.Context(), "workspace-primary", recordtimerapplication.RecordTimerSchedule{
		...
		}, recordmodel.Record{ID: testCase.key, Data: map[string]any{testCase.field: testCase.base.Format(time.RFC3339Nano)}}, nil, now, recordTimerRuntimeSystemScope())
		if err != nil {
