...
func TestRecordTimerHundredThousandRestartClockDriftAndMultiInstanceNoLoss(t *testing.T) {
	...
	preRestart := newRecordTimerRuntimeTestService(t, store, objects)
	preRestart.Applications().RecordTimers.ConfigureWorker(recordtimerapplication.WorkerConfig{Enabled: true, LeaseTTL: 5 * time.Minute, BatchSize: 500})
	lostLeases, err := preRestart.Applications().RecordTimers.ClaimDueRecordTimers(t.Context(), "workspace-primary", now.Add(2*time.Minute), 250, recordTimerRuntimeSystemScope())
	...
	for index := range instances {
		instances[index] = newRecordTimerRuntimeTestService(t, store, objects)
		instances[index].Applications().RecordTimers.ConfigureWorker(recordtimerapplication.WorkerConfig{Enabled: true, LeaseTTL: 5 * time.Minute, BatchSize: 500})
	}
	...
	for len(seen) < total {
		...
		leases, err := instance.Applications().RecordTimers.ClaimDueRecordTimers(t.Context(), "workspace-primary", now.Add(10*time.Minute).Add(drift), 500, recordTimerRuntimeSystemScope())
		if err != nil {
		...
		if err := instance.Applications().RecordTimers.FinishRecordTimers(t.Context(), "workspace-primary", leases, now.Add(10*time.Minute).Add(drift), recordTimerRuntimeSystemScope()); err != nil {
	...
	if err := instances[0].Applications().RecordTimers.FinishRecordTimer(t.Context(), "workspace-primary", lostLeases[0], now.Add(-2*time.Minute), recordTimerRuntimeSystemScope()); !mutation.IsMutationConflict(err, mutation.MutationConflictLeaseLost) {
