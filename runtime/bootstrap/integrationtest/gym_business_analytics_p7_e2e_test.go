...
func TestGymSixDashboardsUseGenericReportRuntimeWithRLSAndSnapshots(t *testing.T) {
	...
	for _, reportKey := range gymAnalyticsP7ReportKeys() {
		summary, err := integrationReportSummary(t.Context(), environment.service, reportKey, "realtime", manager)
		if err != nil {
	...
	for _, reportKey := range gymAnalyticsP7ReportKeys() {
		snapshot, err := integrationReportSnapshot(t.Context(), environment.service, reportKey, "p7-"+reportKey, manager)
		...
		summary, err := integrationReportSummary(t.Context(), environment.service, reportKey, "snapshot", manager)
...
func gymAnalyticsP7Roles() []accessfixture.Bundle {
	...
	return []accessfixture.Bundle{
		{Key: "manager", Permissions: []string{"runtime.appschema.validate_application_definition", "report.summary.get", "report.snapshots.refresh", "gym_metric_fact.read"}, DataPolicies: []accessfixture.DataPolicyFixture{permission}},
		{Key: "coach", Permissions: []string{"report.summary.get", "gym_metric_fact.read"}, DataPolicies: []accessfixture.DataPolicyFixture{owned}},
		{Key: "advisor", Permissions: []string{"report.summary.get", "gym_metric_fact.read"}, DataPolicies: []accessfixture.DataPolicyFixture{owned}},
	}
}
...
func (environment *gymAnalyticsP7Environment) assertReportMatchesOwnedDetail(t *testing.T, reportKey, roleKey, metricType string) {
	...
	summary, err := integrationReportSummary(t.Context(), environment.service, reportKey, "realtime", principal)
...
func TestGymDashboardTargetVolumeP95IsUnderThreeSeconds(t *testing.T) {
	...
	for _, reportKey := range gymAnalyticsP7ReportKeys()[2:] {
		if _, err := integrationReportSnapshot(t.Context(), environment.service, reportKey, "performance-"+reportKey, manager); err != nil {
	...
	for _, reportKey := range gymAnalyticsP7ReportKeys() {
		...
		for attempt := 0; attempt < 20; attempt++ {
			started := time.Now()
			summary, err := integrationReportSummary(t.Context(), environment.service, reportKey, mode, manager)
