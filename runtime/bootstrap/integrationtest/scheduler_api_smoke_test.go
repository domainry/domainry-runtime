...
func TestSchedulerAPISmokeVerifiesRuntimeOperationsAndEvidence(t *testing.T) {
	...
	if err := store.DB().QueryRowContext(t.Context(), `SELECT COUNT(*) FROM runtime_operations WHERE workspace_id = ? AND status = 'succeeded'`, "default").Scan(&operationCount); err != nil || operationCount != 1 {
