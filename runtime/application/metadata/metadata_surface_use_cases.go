...
func (s *MetadataSchemaApplicationService) PublishedRuntimeSchema(ctx context.Context, principal principalmodel.Principal) (PublishedRuntimeSchemaDTO, error) {
	...
	return PublishedRuntimeSchemaDTO{
		...
		Dictionaries:  append([]metadatamodel.DictionarySchema(nil), snapshot.Dictionaries...),
	}, nil
}

func (s *MetadataSchemaApplicationService) PublishedSurfaceContext(ctx context.Context, surface string, principal principalmodel.Principal) (PublishedSurfaceContextDTO, error) {
	if _, err := principalmodel.QueryScopeForPrincipal(principal); err != nil {
