...
func TestMetadataRuntimeRestorationAuthorizationAndPreparation(t *testing.T) {
	...
	service := NewMetadataRuntimeRestorationApplicationService(runtimeNotificationRepositoryStub{}, runtimeMetadataRepositoryStub{})
...
func TestMetadataRemainingAuthorizedAndSuccessfulValidationPaths(t *testing.T) {
	...
	for name, call := range map[string]func() error{
		...
		"texts": func() error {
			_, err := service.ListLocalizedTexts(t.Context(), metadatamodel.LocalizedTextQuery{}, principalmodel.Principal{})
			return err
		},
		"upsert": func() error {
			_, err := service.UpsertLocalizedText(t.Context(), metadatamodel.LocalizedTextUpsertRequest{}, principalmodel.Principal{})
			return err
		},
	...
	if _, err := validationService.ValidateMetadataDefinitionPayload(t.Context(), "connector", metadatamodel.MetadataDefinitionUpsertRequest{Payload: payload}); err != nil {
		t.Fatal(err)
	}
	...
	if normalized, issues, err := validationService.ValidateMetadataDefinitionRequestPayload(t.Context(), "action", "order.submit", metadatamodel.MetadataDefinitionUpsertRequest{Payload: actionPayload}); err != nil || len(issues) != 0 || len(normalized) == 0 {
...
func TestMetadataSnapshotWatcherNilDependencyTick(t *testing.T) {
	ctx, cancel := context.WithCancel(t.Context())
	done := newMetadataSnapshotWatcherApplicationService(MetadataSnapshotWatcherDependencies{}).Start(ctx, time.Millisecond)
