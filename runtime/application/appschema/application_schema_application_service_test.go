...
type upsertMetadataRepository struct {
	...
	before           appschemamodel.ApplicationDefinition
	saved            appschemamodel.ApplicationDefinition
	request          appschemamodel.ApplicationDefinitionUpsertRequest
	disabled         bool
	versions         []appschemamodel.ApplicationDefinitionVersion
	rollback         appschemamodel.ApplicationDefinition
	audit            auditmodel.AuditEvent
	refreshErrorText string
	refreshCalls     int
}

func (r *upsertMetadataRepository) GetDefinition(context.Context, principalmodel.SystemScope, string, string) (appschemamodel.ApplicationDefinition, bool, error) {
	return r.before, r.before.ResourceKey != "", nil
}

func (r *upsertMetadataRepository) UpsertDefinition(_ context.Context, _ principalmodel.SystemScope, _, _ string, request appschemamodel.ApplicationDefinitionUpsertRequest) (appschemamodel.ApplicationDefinition, error) {
	r.request = request
	return r.saved, nil
}

func (r *upsertMetadataRepository) PublishDefinition(_ context.Context, _ principalmodel.SystemScope, _, _ string, request appschemamodel.ApplicationDefinitionUpsertRequest, audit auditmodel.AuditEvent) (appschemamodel.ApplicationDefinition, error) {
...
func (r *upsertMetadataRepository) ListDefinitionVersions(context.Context, principalmodel.SystemScope, string, string) ([]appschemamodel.ApplicationDefinitionVersion, error) {
	return append([]appschemamodel.ApplicationDefinitionVersion(nil), r.versions...), nil
}

func (r *upsertMetadataRepository) RollbackDefinition(_ context.Context, _ principalmodel.SystemScope, _, _ string, _ appschemamodel.ApplicationDefinitionRollbackRequest, audit auditmodel.AuditEvent) (appschemamodel.ApplicationDefinition, error) {
	r.audit = audit
...
func TestServiceOwnsDefinitionUpsertReloadAndAudit(t *testing.T) {
	repository := &upsertMetadataRepository{
		before: appschemamodel.ApplicationDefinition{ResourceType: "action", ResourceKey: "order.approve", Payload: json.RawMessage(`{"old":true}`)},
		saved:  appschemamodel.ApplicationDefinition{ResourceType: "action", ResourceKey: "order.approve", SchemaVersion: "2", SchemaHash: "hash-2", Payload: json.RawMessage(`{"new":true}`)},
	}
	...
	service := NewApplicationSchemaApplicationService(ApplicationSchemaDependencies{Repository: repository, Runtime: runtime, Workflows: upsertWorkflowInitializer{}, Audit: auditEventFactoryStub{}})
	...
	definition, snapshot, err := service.upsertApplicationDefinitionWithOptions(t.Context(), " action ", " order.approve ", appschemamodel.ApplicationDefinitionUpsertRequest{Payload: json.RawMessage(`{"draft":true}`)}, admin, ApplicationSchemaUpsertDefinitionOptions{
		Normalize: func(_ context.Context, resourceType, resourceKey string, request appschemamodel.ApplicationDefinitionUpsertRequest) (appschemamodel.ApplicationDefinitionUpsertRequest, error) {
...
func TestDefinitionPublishMarksDurableIntentForReconciliationWhenRuntimeRefreshFails(t *testing.T) {
	repository := &upsertMetadataRepository{saved: appschemamodel.ApplicationDefinition{ResourceType: "action", ResourceKey: "order.approve", SchemaVersion: "1", SchemaHash: "hash-1", Payload: json.RawMessage(`{"new":true}`)}}
	want := errors.New("workflow refresh unavailable")
	service := NewApplicationSchemaApplicationService(ApplicationSchemaDependencies{Repository: repository, Runtime: &upsertMetadataRuntime{}, Workflows: upsertWorkflowInitializer{err: want}, Audit: auditEventFactoryStub{}})
	admin := accessfixture.Attach(principalmodel.Principal{Principal: identitysdk.Principal{Known: true, WorkspaceID: "default"}}, accessfixture.Bundle{Permissions: []string{"workspace.admin"}})
	_, _, err := service.upsertApplicationDefinitionWithOptions(t.Context(), "action", "order.approve", appschemamodel.ApplicationDefinitionUpsertRequest{Payload: json.RawMessage(`{"draft":true}`)}, admin, ApplicationSchemaUpsertDefinitionOptions{Normalize: func(_ context.Context, _, _ string, request appschemamodel.ApplicationDefinitionUpsertRequest) (appschemamodel.ApplicationDefinitionUpsertRequest, error) {
		return request, nil
	}})
...
func TestServiceDoesNotDuplicateAuditForDefinitionPublishReplay(t *testing.T) {
	current := appschemamodel.ApplicationDefinition{ResourceType: "automation_rule", ResourceKey: "order.sync", SchemaVersion: "3", SchemaHash: "hash-3", Payload: json.RawMessage(`{"key":"order.sync"}`)}
	repository := &upsertMetadataRepository{before: current, saved: current}
	runtime := &upsertMetadataRuntime{snapshot: appschemamodel.ApplicationSchemaSnapshot{AutomationRules: []automationmodel.AutomationRuleSchema{{Key: "order.sync"}}}}
	service := NewApplicationSchemaApplicationService(ApplicationSchemaDependencies{Repository: repository, Runtime: runtime, Workflows: upsertWorkflowInitializer{}, Audit: auditEventFactoryStub{}})
	admin := accessfixture.Attach(principalmodel.Principal{Principal: identitysdk.Principal{Known: true, WorkspaceID: "default"}}, accessfixture.Bundle{Permissions: []string{"workspace.admin"}})
	_, _, err := service.upsertApplicationDefinitionWithOptions(t.Context(), "automation_rule", "order.sync", appschemamodel.ApplicationDefinitionUpsertRequest{Payload: current.Payload}, admin, ApplicationSchemaUpsertDefinitionOptions{
		Normalize: func(_ context.Context, _, _ string, request appschemamodel.ApplicationDefinitionUpsertRequest) (appschemamodel.ApplicationDefinitionUpsertRequest, error) {
			return request, nil
		},
	})
	if err != nil {
...
func TestServiceRejectsDefinitionUpsertWithoutAdmin(t *testing.T) {
	...
	_, _, err := service.upsertApplicationDefinitionWithOptions(t.Context(), "action", "order.approve", appschemamodel.ApplicationDefinitionUpsertRequest{}, principalmodel.Principal{}, ApplicationSchemaUpsertDefinitionOptions{})
	if err == nil {
...
func TestServiceOwnsDefinitionDisableImpactReloadAndAudit(t *testing.T) {
	repository := &upsertMetadataRepository{before: appschemamodel.ApplicationDefinition{ResourceType: "action", ResourceKey: "order.approve", Payload: json.RawMessage(`{"key":"order.approve"}`)}}
	service := NewApplicationSchemaApplicationService(ApplicationSchemaDependencies{Repository: repository, Runtime: &upsertMetadataRuntime{}, Workflows: upsertWorkflowInitializer{}, Audit: auditEventFactoryStub{}})
	admin := accessfixture.Attach(principalmodel.Principal{Principal: identitysdk.Principal{Known: true, WorkspaceID: "default"}}, accessfixture.Bundle{Permissions: []string{"workspace.admin"}})
	audited := false
	err := service.disableApplicationDefinitionWithOptions(t.Context(), " action ", " order.approve ", admin, ApplicationSchemaDisableDefinitionOptions{
...
func TestServiceOwnsDefinitionRollbackContractRepositoryReloadAndAudit(t *testing.T) {
	repository := &upsertMetadataRepository{
		before:   appschemamodel.ApplicationDefinition{ResourceType: "action", ResourceKey: "order.approve", SchemaVersion: "3", SchemaHash: "hash-3", Payload: json.RawMessage(`{"version":3}`)},
		versions: []appschemamodel.ApplicationDefinitionVersion{{SchemaVersion: "2", SchemaHash: "hash-2", Payload: json.RawMessage(`{"version":2}`)}},
		rollback: appschemamodel.ApplicationDefinition{ResourceType: "action", ResourceKey: "order.approve", SchemaVersion: "2", SchemaHash: "hash-2"},
	}
	service := NewApplicationSchemaApplicationService(ApplicationSchemaDependencies{Repository: repository, Runtime: &upsertMetadataRuntime{}, Workflows: upsertWorkflowInitializer{}, Audit: auditEventFactoryStub{}})
	...
	definition, _, err := service.rollbackApplicationDefinitionWithOptions(t.Context(), " action ", " order.approve ", request, admin, ApplicationSchemaRollbackDefinitionOptions{
		AuthoringContractVersion: "authoring-v1", AuthoringContractHash: "contract-hash",
