...
func TestPublishedSurfaceContextIsBoundToBackendPrincipalSurface(t *testing.T) {
	service := NewApplicationSchemaQueryApplicationService(metadataSchemaApplicationProviderStub{snapshot: appschemamodel.ApplicationSchemaSnapshot{SchemaHash: "hash"}}, nil)
...
func TestMetadataSurfaceUseCasesRejectMissingScopeAndCoverOptionalContext(t *testing.T) {
	schemaService := NewApplicationSchemaQueryApplicationService(
		metadataSchemaApplicationProviderStub{snapshot: appschemamodel.ApplicationSchemaSnapshot{SchemaHash: "hash"}},
		nil,
	)
...
func TestOpsMetadataDiagnosticsRejectsEveryUnavailableDependency(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "nil service"},
		{name: "nil repository", service: NewApplicationSchemaApplicationService(ApplicationSchemaDependencies{Runtime: runtime})},
		{name: "nil runtime", service: NewApplicationSchemaApplicationService(ApplicationSchemaDependencies{Repository: repository})},
	}
