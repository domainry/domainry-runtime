...
type ApplicationSchemaSnapshotWatcherDependencies struct {
	Revision func(context.Context) (string, error)
	Reload   func(context.Context) error
}

type applicationSchemaSnapshotWatcherApplicationService struct {
	dependencies ApplicationSchemaSnapshotWatcherDependencies
}

func newApplicationSchemaSnapshotWatcherApplicationService(dependencies ApplicationSchemaSnapshotWatcherDependencies) *applicationSchemaSnapshotWatcherApplicationService {
	return &applicationSchemaSnapshotWatcherApplicationService{dependencies: dependencies}
}

func (w *applicationSchemaSnapshotWatcherApplicationService) Start(ctx context.Context, interval time.Duration) <-chan struct{} {
	if interval <= 0 {
