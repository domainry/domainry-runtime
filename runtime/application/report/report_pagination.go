...
func (s *ReportApplicationService) SummaryScoped(ctx context.Context, reportKey, mode, queryKey string, tags []string, principal principalmodel.Principal) (reportmodel.ReportSummary, error) {
...
func (s *ReportApplicationService) SummaryScopedPage(ctx context.Context, reportKey, mode, queryKey string, tags []string, page reportmodel.ReportPageRequest, principal principalmodel.Principal) (reportmodel.ReportSummary, error) {
	if _, err := principalmodel.QueryScopeForPrincipal(principal); err != nil {
		return reportmodel.ReportSummary{}, reportWorkspaceError(err)
	}
	report, err := s.domain.ReportForSummary(ctx, reportKey, principal)
