...
func TestFullAgentSchemaUsesInternalSnapshotWithoutChangingAnonymousProjection(t *testing.T) {
	provider := agentInternalSchemaProviderStub{
		...
		internal:                appschemamodel.ApplicationSchemaSnapshot{AgentServicePrincipals: []agentmodel.AgentServicePrincipalBinding{{Key: "service", Enabled: true}}},
	}
...
func TestAgentExecutionIdentityRefreshesAndSeparatesInitiator(t *testing.T) {
	...
	execution, identity, err := service.ResolveExecutionIdentity(t.Context(), AgentExecutionIdentityRequest{Initiator: initiator, Identity: agentmodel.AgentTaskIdentity{Mode: agentmodel.AgentTaskIdentityInherit}})
	if err != nil {
		t.Fatal(err)
	}
	...
	if identity.Initiator.UserID != identity.Execution.UserID || identity.Mode != agentmodel.AgentTaskIdentityInherit {
		t.Fatalf("inherit identity changed actor: %#v", identity)
	}

	execution, identity, err = service.ResolveExecutionIdentity(t.Context(), AgentExecutionIdentityRequest{Initiator: initiator, Identity: agentmodel.AgentTaskIdentity{Mode: agentmodel.AgentTaskIdentityService, PrincipalKey: "customer_service"}, ExpectedRotationVersion: 3})
	if err != nil {
		t.Fatal(err)
	}
...
func TestAgentExecutionIdentityFailsClosedOnRevocationAndRotation(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "unknown initiator", mutate: func() {}, req: AgentExecutionIdentityRequest{Identity: agentmodel.AgentTaskIdentity{Mode: agentmodel.AgentTaskIdentityInherit}}, code: "agent.authorization.initiator_inactive"},
		...
		}, req: AgentExecutionIdentityRequest{Initiator: initiator, Identity: agentmodel.AgentTaskIdentity{Mode: agentmodel.AgentTaskIdentityInherit}}, code: "agent.authorization.execution_principal_inactive"},
		...
		}, req: AgentExecutionIdentityRequest{Initiator: initiator, Identity: agentmodel.AgentTaskIdentity{Mode: agentmodel.AgentTaskIdentityInherit}}, code: "agent.authorization.workspace_removed"},
		{name: "service disabled", mutate: func() { schema.full.AgentServicePrincipals[0].Enabled = false }, req: AgentExecutionIdentityRequest{Initiator: initiator, Identity: agentmodel.AgentTaskIdentity{Mode: agentmodel.AgentTaskIdentityService, PrincipalKey: "customer_service"}}, code: "agent.authorization.service_principal_disabled"},
		{name: "rotation stale", mutate: func() {}, req: AgentExecutionIdentityRequest{Initiator: initiator, Identity: agentmodel.AgentTaskIdentity{Mode: agentmodel.AgentTaskIdentityService, PrincipalKey: "customer_service"}, ExpectedRotationVersion: 2}, code: "agent.authorization.service_rotation_stale"},
		{name: "unknown mode", mutate: func() {}, req: AgentExecutionIdentityRequest{Initiator: initiator, Identity: agentmodel.AgentTaskIdentity{Mode: "root"}}, code: "agent.authorization.identity_mode_denied"},
	}
	...
	if _, _, err := unavailable.ResolveExecutionIdentity(t.Context(), AgentExecutionIdentityRequest{Initiator: initiator, Identity: agentmodel.AgentTaskIdentity{Mode: agentmodel.AgentTaskIdentityInherit}}); apperror.CodeOf(err) != "agent.authorization.resolver_unavailable" {
		t.Fatalf("missing resolver error = %v", err)
	}
	failure := errors.New("directory offline")
	service.principals = agentPrincipalDirectoryStub{err: failure}
	if _, _, err := service.ResolveExecutionIdentity(t.Context(), AgentExecutionIdentityRequest{Initiator: initiator, Identity: agentmodel.AgentTaskIdentity{Mode: agentmodel.AgentTaskIdentityInherit}}); !errors.Is(err, failure) {
...
func TestAgentTaskAuthorizationIntersectsEveryCapabilityLayer(t *testing.T) {
	...
	authorized, err := service.AuthorizeTask(t.Context(), AgentTaskAuthorizationRequest{
		Initiator: initiator, Identity: agentmodel.AgentTaskIdentity{Mode: agentmodel.AgentTaskIdentityInherit}, TaskKey: "customer.review", TaskVersion: "1.0.0",
	...
	request := AgentTaskAuthorizationRequest{Initiator: initiator, Identity: agentmodel.AgentTaskIdentity{Mode: agentmodel.AgentTaskIdentityInherit}, TaskKey: "customer.review", TaskVersion: "1.0.0"}
...
func TestAgentExecutionIdentityBoundaryMatrix(t *testing.T) {
	...
	for name, candidate := range map[string]principalmodel.Principal{
		...
		if _, _, err := service.ResolveExecutionIdentity(t.Context(), AgentExecutionIdentityRequest{Initiator: candidate, Identity: agentmodel.AgentTaskIdentity{Mode: agentmodel.AgentTaskIdentityInherit}}); apperror.CodeOf(err) != "agent.authorization.initiator_inactive" {
	...
	for name, candidate := range map[string]*AgentAuthorizationApplicationService{
		...
		if _, _, err := candidate.ResolveExecutionIdentity(t.Context(), AgentExecutionIdentityRequest{Initiator: initiator, Identity: agentmodel.AgentTaskIdentity{Mode: agentmodel.AgentTaskIdentityInherit}}); apperror.CodeOf(err) != "agent.authorization.resolver_unavailable" {
			t.Fatalf("%s=%v", name, err)
		}
	}
	if _, _, err := service.ResolveExecutionIdentity(t.Context(), AgentExecutionIdentityRequest{Initiator: initiator, Identity: agentmodel.AgentTaskIdentity{Mode: agentmodel.AgentTaskIdentityService, PrincipalKey: "missing"}}); apperror.CodeOf(err) != "agent.authorization.service_principal_disabled" {
		t.Fatalf("missing service=%v", err)
	}
	if _, _, err := service.ResolveExecutionIdentity(t.Context(), AgentExecutionIdentityRequest{Initiator: initiator, Identity: agentmodel.AgentTaskIdentity{Mode: agentmodel.AgentTaskIdentityService, PrincipalKey: "customer_service"}}); err != nil {
		t.Fatalf("zero expected rotation=%v", err)
	}
	directory.err = errors.New("service directory")
	if _, _, err := service.ResolveExecutionIdentity(t.Context(), AgentExecutionIdentityRequest{Initiator: initiator, Identity: agentmodel.AgentTaskIdentity{Mode: agentmodel.AgentTaskIdentityService, PrincipalKey: "customer_service"}}); !errors.Is(err, directory.err) {
	...
	for name, workspace := range map[string]string{"invalid": "", "mismatch": "other"} {
		...
		directory.principals["operator:operator"] = value
		if _, _, err := service.ResolveExecutionIdentity(t.Context(), AgentExecutionIdentityRequest{Initiator: initiator, Identity: agentmodel.AgentTaskIdentity{Mode: agentmodel.AgentTaskIdentityInherit}}); apperror.CodeOf(err) != "agent.authorization.workspace_removed" {
...
func TestAgentTaskAuthorizationBoundaryMatrix(t *testing.T) {
	baseRequest := func(initiator principalmodel.Principal) AgentTaskAuthorizationRequest {
		return AgentTaskAuthorizationRequest{Initiator: initiator, Identity: agentmodel.AgentTaskIdentity{Mode: agentmodel.AgentTaskIdentityInherit}, TaskKey: "customer.review", TaskVersion: "1.0.0"}
	}
	service, initiator, schema, _ := agentAuthorizationFixture()
	...
	task.SideEffectMode = agentmodel.AgentTaskSideEffectAnalysisOnly
	...
	if tools := agentToolsForTask(appschemamodel.ApplicationSchemaSnapshot{Agents: []agentmodel.AgentSchema{{Key: "other"}}, Skills: []agentmodel.SkillSchema{{Key: "unused", AllowedTools: []string{"x"}}}}, task); len(tools) != 0 {
		t.Fatalf("unmatched tools=%v", tools)
	}
	local, localInitiator, localSchema, _ = agentAuthorizationFixture()
	emptyTask := localSchema.full.AgentTasks[0]
	emptyTask.AllowedObjects, emptyTask.AllowedActions, emptyTask.SideEffectMode = nil, nil, agentmodel.AgentTaskSideEffectAnalysisOnly
...
func TestGlobalAgentContextBoundaryMatrix(t *testing.T) {
	...
	for name, test := range map[string]struct {
		...
		"route surface": {func(_ *AgentAuthorizationApplicationService, s *agentSchemaProviderStub, _ *GlobalAgentContextRequest) {
			s.full.EntryPoints[0].Config["kind"] = "admin_console"
		}, "agent.authorization.route_denied"},
		"visible route": {func(_ *AgentAuthorizationApplicationService, s *agentSchemaProviderStub, _ *GlobalAgentContextRequest) {
	...
	schema.full.EntryPoints[0].Config = map[string]any{"kind": "backoffice", "read_objects": []any{"customer"}}
	if _, err := service.ResolveGlobalContext(t.Context(), base(initiator)); err != nil {
		t.Fatalf("any object config=%v", err)
	}
...
func TestAgentAuthorizationLookupAndCollectionHelpers(t *testing.T) {
	...
	if _, found := findAgentTask([]agentmodel.AgentTaskDefinition{{Key: "other", Version: "1"}, {Key: "x", Version: "2"}}, "x", "1"); found {
		t.Fatal("mismatched task found")
	}
	if _, found := findAgentEntrypoint(nil, "x"); found {
		t.Fatal("entrypoint found")
	}
	if _, found := findAgentEntrypoint([]agentmodel.AgentEntrypointAssignment{{Key: "other"}}, "x"); found {
		t.Fatal("mismatched entrypoint found")
	}
	if _, found := findBusinessEntrypoint(nil, "x"); found {
		t.Fatal("business entrypoint found")
	}
	...
	for kind, want := range map[string]string{"backoffice": "business_workspace", "operator_console": "business_workspace", "business_workspace": "business_workspace", "admin_console": "admin_console", "customer_portal": "consumer_portal", "consumer_portal": "consumer_portal", "unknown": ""} {
		if got := agentEntrypointSurface(definitionmodel.EntryPointSchema{Config: map[string]any{"kind": kind}}); got != want {
			t.Fatalf("surface %s=%q", kind, got)
		}
	}
	...
	if len(agentTaskKeys([]agentmodel.AgentTaskDefinition{{Key: "off"}})) != 0 || len(agentWorkflowKeys([]definitionmodel.WorkflowSchema{{Key: "off"}})) != 0 {
...
func TestAgentInteractiveAuthorizationBoundaryMatrix(t *testing.T) {
	...
	local, localInitiator, localSchema, _ := agentAuthorizationFixture()
	localInitiator.SurfaceKey = "business_workspace"
	localSchema.full.AgentEntrypoints[0].RoutingContract.AllowedRouteTypes = []string{agentmodel.AgentRouteInteractiveQuery, agentmodel.AgentRouteTask, agentmodel.AgentRouteWorkflow, agentmodel.AgentRouteProposal}
	...
	visible := localSchema.filtered["operator:operator"]
	visible.AgentTasks = append(visible.AgentTasks, agentmodel.AgentTaskDefinition{Key: "disabled", Enabled: false}, agentmodel.AgentTaskDefinition{Key: "other", Enabled: true})
	...
	if tools := agentToolsForInteractiveAgent(appschemamodel.ApplicationSchemaSnapshot{Skills: []agentmodel.SkillSchema{{Key: "other", AllowedTools: []string{"x"}}}}, agentmodel.AgentSchema{SkillKeys: []string{"wanted"}}); len(tools) != 0 {
...
func TestAgentInteractiveAuthorizationDetectsMidRequestIdentityAndSchemaChanges(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			stored := agentmodel.GlobalAgentContext{EntrypointKey: "assistant.global", AgentKey: "customer_agent", Surface: "business_workspace", RouteKey: "workspace.customer", ObjectKey: "customer"}
	...
	for name, mutate := range map[string]func(*appschemamodel.ApplicationSchemaSnapshot){
		...
		t.Run(name, func(t *testing.T) {
			...
			initiator.SurfaceKey = "business_workspace"
			second := schema.full
			second.AgentEntrypoints = append([]agentmodel.AgentEntrypointAssignment(nil), schema.full.AgentEntrypoints...)
			mutate(&second)
