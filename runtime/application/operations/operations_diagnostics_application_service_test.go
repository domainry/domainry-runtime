...
func TestOperationsDiagnosticsIsBoundedAndReceiptReplayDoesNotRecapture(t *testing.T) {
	...
	principal := accessfixture.Attach(principalmodel.Principal{Principal: identitysdk.Principal{Known: true, WorkspaceID: "workspace-a", UserID: "admin"}}, accessfixture.Bundle{Permissions: []string{"runtime.operations.capture_diagnostics"}})
