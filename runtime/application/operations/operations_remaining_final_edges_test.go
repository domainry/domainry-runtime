...
func TestOperationsOwnerExecutionFinalDefinitionAndReceiptEdges(t *testing.T) {
	...
	if _, err := service.ExecuteOwnerOperation(t.Context(), OperationsOwnerExecutionRequest{Kind: "scheduler.run.retry", ResourceType: "wrong"}, principal, func(context.Context) (any, error) { return nil, nil }); apperror.CodeOf(err) != "backend.operations.definition_mismatch" {
		t.Fatalf("resource mismatch error = %v", err)
	}

	request := OperationsOwnerExecutionRequest{Kind: "scheduler.run.retry", ResourceType: "scheduler_run", ResourceID: "run", Reason: "test", Key: "empty-result"}
	ledger := &operationsRepositoryProbe{receipts: map[string]operationsmodel.OperationsReceipt{}}
	...
	if _, _, err := service.Submit(t.Context(), OperationsSubmitRequest{Kind: createdRequest.Kind, Permission: "workspace.admin", ResourceType: createdRequest.ResourceType, ResourceID: createdRequest.ResourceID, Reason: createdRequest.Reason}, createdRequest.Key, principal); err != nil {
		t.Fatal(err)
	}
	...
	if _, _, err := service.Submit(t.Context(), OperationsSubmitRequest{Kind: unknownStatusRequest.Kind, Permission: "workspace.admin", ResourceType: unknownStatusRequest.ResourceType, ResourceID: unknownStatusRequest.ResourceID, Reason: unknownStatusRequest.Reason}, unknownStatusRequest.Key, principal); err != nil {
		t.Fatal(err)
	}
