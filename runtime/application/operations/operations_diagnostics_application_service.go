...
func (s *OperationsApplicationService) CaptureDiagnostics(ctx context.Context, command OperationsDiagnosticsCommand, key string, principal principalmodel.Principal) (OperationsDiagnosticsResult, error) {
	...
	receipt, decision, err := s.SubmitSystem(ctx, OperationsSubmitRequest{Kind: "diagnostics.snapshot", Permission: "workspace.admin", ResourceType: "runtime", ResourceID: s.instanceID, Reason: command.Reason, Reference: command.Reference, Payload: command}, key, operationsmodel.OperationsSystemPurposeRuntimeControl, principal)
	if err != nil {
