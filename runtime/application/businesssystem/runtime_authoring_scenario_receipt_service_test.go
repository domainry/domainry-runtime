...
func TestRuntimeAuthoringScenarioReceiptRejectsTamperingAndStaleBinding(t *testing.T) {
	...
	receipt, err := service.Issue(RuntimeAuthoringScenarioStepObservation{
		...
		Method: "post", Path: "/records/order", ExpectedStatus: []int{201, 201}, ActualStatus: 201,
...
func TestRuntimeAuthoringEvidenceSessionIssuesPlanBoundStepTokens(t *testing.T) {
	...
	plan := changeplanmodel.RuntimeAuthoringEvidencePlan{
		Scenarios: []changeplanmodel.RuntimeAuthoringEvidenceScenarioPlan{{
			...
			Steps: []changeplanmodel.RuntimeAuthoringEvidenceStepPlan{{StepID: "create", Label: "create order", Method: "post", Path: "/records/order", ExpectedStatus: []int{201}}},
		}},
	}
...
func TestRuntimeAuthoringDeliveryTrustsOnlyRuntimeIssuedStepReceipts(t *testing.T) {
	...
	plan := changeplanmodel.RuntimeAuthoringEvidencePlan{
		Scenarios: []changeplanmodel.RuntimeAuthoringEvidenceScenarioPlan{{
			ScenarioID: "order.lifecycle", Categories: append([]string(nil), changeplanmodel.RuntimeAuthoringRequiredScenarioCategories...),
			Steps: []changeplanmodel.RuntimeAuthoringEvidenceStepPlan{{StepID: "registered-step", Label: "registered step", Method: "GET", Path: "/records/order/items/order-1", ExpectedStatus: []int{200}}},
		}},
	}
	...
	steps := []changeplanmodel.RuntimeAuthoringScenarioStepEvidence{
		{RuntimeReceipt: issue("success", "/records/order/items/order-1", 200, strings.Repeat("3", 64), "", "", false), ActualStatus: 500, Passed: false},
		{RuntimeReceipt: issue("denied", "/records/order/items/order-1", 403, strings.Repeat("4", 64), "", "", false)},
		{RuntimeReceipt: issue("precondition", "/records/order/items/order-1/actions/complete", 422, strings.Repeat("5", 64), "", "", false)},
		{RuntimeReceipt: issue("before", "/records/order/items/order-1", 200, stateHash, "before_state", "", false)},
		{RuntimeReceipt: issue("after", "/records/order/items/order-1", 200, stateHash, "after_state", "", false)},
		{RuntimeReceipt: issue("replay-one", "/records/order", 200, strings.Repeat("6", 64), "", "create-1", false)},
		{RuntimeReceipt: issue("replay-two", "/records/order", 200, strings.Repeat("6", 64), "", "create-1", true)},
		{RuntimeReceipt: issue("audit", "/audit/events", 200, strings.Repeat("7", 64), "", "", false)},
		{RuntimeReceipt: issue("event", "/realtime/refresh-events", 200, strings.Repeat("8", 64), "", "", false)},
