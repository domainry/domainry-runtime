...
func TestRecordEffectAuthorizationPrincipalScopesCreateAndRejectsOtherVerbs(t *testing.T) {
	principal := accessfixture.Attach(principalmodel.Principal{Principal: identitysdk.Principal{Known: true, WorkspaceID: "workspace-a", UserID: "member"}}, accessfixture.Bundle{Key: "member", Permissions: []string{"application.register"}, DataPolicies: accessfixture.DataPoliciesForPermissions([]string{"application.register"}, identitysdk.DataScopeAll)})
	ctx := recordmutation.WithMutationInvocation(t.Context(), recordmutation.MutationInvocation{
		Source: transactionmodel.MutationSourceAction, ActionKey: "application.register", ActionResource: "application", ActionOperation: "register", EffectAuthority: map[string][]string{"application": {"number", "status", "policy_value"}},
	})
...
func TestActionEffectAuthorityAllowsInternalUpdateWithoutObjectUpdatePermission(t *testing.T) {
	...
	ctx := recordmutation.WithMutationInvocation(t.Context(), recordmutation.MutationInvocation{
		Source: transactionmodel.MutationSourceAction, ActionKey: "order.approve", ActionResource: "order", ActionOperation: "approve", EffectAuthority: map[string][]string{"order": {"status"}},
	})
...
func TestActionEffectAuthorityAllowsGovernedConditionalUpdateWithoutObjectUpdatePermission(t *testing.T) {
	...
	service := NewRecordUpdateApplicationService(dependencies)
	principal := accessfixture.Attach(principalmodel.Principal{Principal: identitysdk.Principal{Known: true, WorkspaceID: "workspace-a", UserID: "approver"}}, accessfixture.Bundle{Key: "approver", Permissions: []string{"order.approve"}, DataPolicies: accessfixture.DataPoliciesForPermissions([]string{"order.approve"}, identitysdk.DataScopeAll)})
	ctx := recordmutation.WithMutationInvocation(t.Context(), recordmutation.MutationInvocation{
		Source: transactionmodel.MutationSourceAction, ActionKey: "order.approve", ActionResource: "order", ActionOperation: "approve",
...
func TestActionEffectAuthorityAllowsGovernedInternalCreateWithoutObjectCreatePermission(t *testing.T) {
	...
	principal := accessfixture.Attach(principalmodel.Principal{Principal: identitysdk.Principal{Known: true, WorkspaceID: "workspace-a", UserID: "member"}}, accessfixture.Bundle{
		Key: "member", Permissions: []string{"customer.register"}, DataPolicies: accessfixture.DataPoliciesForPermissions([]string{"customer.register"}, identitysdk.DataScopeAll),
	...
	ctx := recordmutation.WithMutationInvocation(t.Context(), recordmutation.MutationInvocation{
		Source: transactionmodel.MutationSourceAction, ActionKey: "customer.register", ActionResource: "customer", ActionOperation: "register", EffectAuthority: map[string][]string{"customer": {"mobile", "status", "policy_value"}},
	})
