...
func TestInternalMutationUpdateAndRepositoryFailures(t *testing.T) {
	...
	if err := service.Update(t.Context(), "workspace-a", RecordInternalMutationOwnerPathRebuild, object, record, "runtime progress"); err != nil || repository.updated.ID != record.ID {
		t.Fatalf("updated=%#v err=%v", repository.updated, err)
	}

	failure := errors.New("store unavailable")
	repository.updateErr = failure
	if err := service.Update(t.Context(), "workspace-a", RecordInternalMutationOwnerPathRebuild, object, record, "runtime progress"); apperror.CodeOf(err) != "backend.internal" || apperror.ParamsOf(err)["operation"] != "internal record update" || !errors.Is(err, failure) {
...
func TestInternalMutationServiceRejectsDisallowedPoliciesBeforeRepository(t *testing.T) {
	...
	unknown := RecordInternalMutationPolicy("unknown")
	if err := service.Insert(t.Context(), "workspace-a", unknown, object, record, "invalid"); apperror.CodeOf(err) != "backend.record.internal_mutation_policy_denied" {
		t.Fatalf("insert policy err=%v", err)
	}
	if err := service.Update(t.Context(), "workspace-a", unknown, object, record, "invalid"); apperror.CodeOf(err) != "backend.record.internal_mutation_policy_denied" {
