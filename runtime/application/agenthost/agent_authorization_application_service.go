...
func fullAgentSchema(ctx context.Context, provider AgentSchemaProvider) appschemamodel.ApplicationSchemaSnapshot {
...
func (s *AgentAuthorizationApplicationService) ResolveGlobalContext(ctx context.Context, request GlobalAgentContextRequest) (result agentsdk.GlobalContext, err error) {
	ctx, span := telemetry.StartUseCase(ctx, "agent.global_context.resolve", attribute.String("workspace.id", request.Principal.WorkspaceID), attribute.String("agent.entrypoint", request.EntrypointKey), attribute.String("route.key", request.RouteKey))
	...
	contextValue := agentsdk.GlobalContext{
		ContractVersion: agentsdk.GlobalAgentContextContractVersion, EntrypointKey: assignment.Key, AgentKey: assignment.AgentKey, RouteKey: strings.TrimSpace(request.RouteKey),
