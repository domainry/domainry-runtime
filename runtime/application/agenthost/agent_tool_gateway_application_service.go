...
const (
	AgentToolQueryRecords = agentsdk.AgentToolQueryRecords
	AgentToolGetRecord    = agentsdk.AgentToolGetRecord
	AgentToolInvokeAction = agentsdk.AgentToolInvokeAction
)
...
func (g *AgentToolGateway) Invoke(ctx context.Context, request AgentToolInvocationRequest) (result AgentToolInvocationResult, err error) {
	...
	if g == nil || g.dependencies.Authorization == nil || g.dependencies.Credentials == nil {
	...
	if _, supported := agentsdk.LookupAgentTool(request.Tool); !supported {
	...
	result = AgentToolInvocationResult{Tool: request.Tool, Authorization: authorization.Evidence}
	...
	case AgentToolInvokeAction:
		...
		if authorization.Task.SideEffectMode == agentsdk.AgentTaskSideEffectProposalOnly || requiresProposal {
			result.Proposal = agentProposalDraft(actionKey, objectKey, recordID, data, "agent_task", request.TaskRunID, map[string]any{
				"process_id": request.ProcessID, "task_run_id": request.TaskRunID,
				"idempotency_key": request.IdempotencyKey, "execution_mode": authorization.Identity.Mode,
				"service_principal_key": authorization.Identity.ServicePrincipalKey,
			})
...
func agentProposalDraft(actionKey, objectKey, recordID string, input map[string]any, source, reference string, metadata map[string]any) map[string]any {
	return map[string]any{
		"title": "Agent action proposal: " + strings.TrimSpace(actionKey), "source": strings.TrimSpace(source), "reference": strings.TrimSpace(reference),
		"proposed": map[string]any{"action_binding": map[string]any{
			"action_key": strings.TrimSpace(actionKey), "object_key": strings.TrimSpace(objectKey), "record_id": strings.TrimSpace(recordID),
			"data": agentToolMap(input), "guarded_write": true,
		}},
		"metadata": metadata,
	}
}
