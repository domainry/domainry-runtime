...
func TestRecordsCRUDSuccessReplayAndNilPatch(t *testing.T) {
	...
	created := fixture.call(http.MethodPost, "/records/customer", `{"data":{"name":"Ada"}}`, map[string]string{"Idempotency-Key": "record-create-edge"})
	...
	replayed := fixture.call(http.MethodPost, "/records/customer", `{"data":{"name":"Ada"}}`, map[string]string{"Idempotency-Key": "record-create-edge"})
	...
	missingUpdateKey := fixture.call(http.MethodPatch, "/records/customer/items/"+record.ID, `{"data":{"name":"Grace","expected_updated_at":"`+record.UpdatedAt+`"}}`, nil)
	...
	updated := fixture.call(http.MethodPatch, "/records/customer/items/"+record.ID, `{"data":{"name":"Grace","expected_updated_at":"`+record.UpdatedAt+`"}}`, map[string]string{"Idempotency-Key": "record-update-edge"})
	...
	missingDeleteKey := fixture.call(http.MethodDelete, "/records/customer/items/"+record.ID+"?expected_updated_at="+record.UpdatedAt, "", nil)
	...
	deleted := fixture.call(http.MethodDelete, "/records/customer/items/"+record.ID+"?expected_updated_at="+record.UpdatedAt, "", map[string]string{"Idempotency-Key": "record-delete-edge"})
	if deleted.Code != http.StatusNoContent {
		t.Fatalf("delete status=%d body=%s", deleted.Code, deleted.Body.String())
	}
	replayedDelete := fixture.call(http.MethodDelete, "/records/customer/items/"+record.ID+"?expected_updated_at="+record.UpdatedAt, "", map[string]string{"Idempotency-Key": "record-delete-edge"})
...
func TestRecordCreateMissingKeyUsesOperationsEnvelopeContract(t *testing.T) {
	fixture := newRecordBatchHTTPFixture(t)
	response := fixture.call(http.MethodPost, "/records/customer", `{"data":{"name":"Ada"}}`, nil)
...
func TestRecordsImportAndBatchReplayFailureEdges(t *testing.T) {
	...
	applyPath := "/records/customer/import/apply"
