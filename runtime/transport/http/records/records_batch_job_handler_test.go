...
func newRecordBatchHTTPFixture(t *testing.T) recordBatchHTTPFixture {
	...
	services := runtimetestkit.NewRuntimeServices(t.Context(), runtimetestkit.RuntimeServicesConfig{
		...
		Objects: []definitionmodel.ObjectSchema{object}, Store: store,
	})
	...
	return recordBatchHTTPFixture{handler: handler, mux: mux, repository: recordpersistence.NewRecordStore(store), principal: principal}
}
...
func TestRecordBatchJobHTTPExportLifecycleAndWorkspaceIsolation(t *testing.T) {
	...
	stored, found, err := fixture.repository.GetRecordBatchJob(t.Context(), "workspace-a", job.ID)
	if err != nil || !found {
		t.Fatalf("stored job found=%v err=%v", found, err)
	}
	var payload struct {
		Options recordapplication.RecordExportOptions `json:"options"`
	}
	if err := json.Unmarshal([]byte(stored.PayloadJSON), &payload); err != nil || len(payload.Options.Fields) != 1 || payload.Options.Fields[0] != "name" || payload.Options.Reason != "audit" || payload.Options.MaskingPolicy != "strict" || payload.Options.FilterSummary != "active" || payload.Options.Query.Page != 2 || payload.Options.Query.PageSize != 25 {
...
func TestRecordBatchJobHTTPDownloadStreamsChunksAndStopsOnTransportSignals(t *testing.T) {
	...
	var queued recordmodel.RecordBatchJob
	if created.Code != http.StatusAccepted || json.Unmarshal(created.Body.Bytes(), &queued) != nil {
		t.Fatalf("enqueue status=%d body=%s", created.Code, created.Body.String())
	}
	claimed, err := fixture.repository.ClaimRecordBatchJobs(t.Context(), 1, "download-worker", time.Minute, time.Now())
	if err != nil || len(claimed) != 1 {
		t.Fatalf("claimed=%+v err=%v", claimed, err)
	}
	job := claimed[0]
	chunks := []recordmodel.RecordBatchJobChunk{{Content: "name\n"}, {Content: "Acme\n"}}
	if err := fixture.repository.ReplaceRecordBatchJobChunks(t.Context(), job, chunks, time.Now()); err != nil {
		t.Fatal(err)
	}
	job.ResultFilename, job.ResultType, job.ResultChunks = "customer.csv", "text/csv", len(chunks)
	if err := fixture.repository.CompleteRecordBatchJob(t.Context(), job, time.Now()); err != nil {
		t.Fatal(err)
	}
