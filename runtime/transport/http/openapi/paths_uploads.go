...
func addUploadOpenAPIPaths(paths map[string]any) {
	paths["/uploads"] = map[string]any{"post": openAPIRuntimeClient(openAPIOperation(
	...
	paths["/uploads/{fileID}/scan"] = map[string]any{"get": openAPIRuntimeClient(openAPIOperation("getFileScan", "Files", "Read Runtime-owned scan status and a signed receipt only when clean", openAPIAdminSecurity(), openAPIPathParameter("fileID", "Runtime file identity"), openAPIJSONResponse("File scan evidence", openAPIObject(nil))), "getFileScan")}
	paths["/uploads/{fileID}"] = map[string]any{
		"get": openAPIRuntimeClient(openAPIOperation("downloadUpload", "Files", "Download a clean uploaded file by immutable Runtime file identity under its authorized object/field/record scope", openAPIAdminSecurity(), openAPIPathParameter("fileID", "Immutable Runtime file identity"), openAPIRequiredQueryParameter("object_key", "Object owning the file field", map[string]any{"type": "string"}), openAPIRequiredQueryParameter("field_key", "Authorized file field", map[string]any{"type": "string"}), openAPIQueryParameter("record_id", "Record that must structurally reference this file identity", map[string]any{"type": "string"}), openAPIResponse("File", "application/octet-stream", map[string]any{"type": "string", "format": "binary"})), "downloadUpload"),
	}
}
