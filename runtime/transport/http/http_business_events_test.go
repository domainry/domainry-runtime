...
func TestBusinessEventPublicationMiddlewarePublishesOnlySuccessfulAuthenticatedMutations(t *testing.T) {
	...
	request := requestWithPrincipal(httptest.NewRequest(http.MethodPost, "/records/customer", nil), principal)
	...
	failed.ServeHTTP(httptest.NewRecorder(), requestWithPrincipal(httptest.NewRequest(http.MethodPost, "/records/customer", nil), principal))
