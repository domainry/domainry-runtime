...
type localizedTextHandlerRepository struct {
	...
	upsertErr    error
	failUpsertAt int
	queries      []metadatamodel.LocalizedTextQuery
	upserts      []metadatamodel.LocalizedTextUpsertRequest
}

func (r *localizedTextHandlerRepository) ListLocalizedTexts(_ context.Context, workspaceID string, query metadatamodel.LocalizedTextQuery) ([]metadatamodel.LocalizedText, error) {
	query.WorkspaceID = workspaceID
	r.queries = append(r.queries, query)
	if r.listErr != nil {
		return nil, r.listErr
	}
	return append([]metadatamodel.LocalizedText(nil), r.values...), nil
}

func (r *localizedTextHandlerRepository) UpsertLocalizedText(_ context.Context, workspaceID string, req metadatamodel.LocalizedTextUpsertRequest) (metadatamodel.LocalizedText, error) {
	req.WorkspaceID = workspaceID
	r.upserts = append(r.upserts, req)
	if r.upsertErr != nil || r.failUpsertAt > 0 && len(r.upserts) == r.failUpsertAt {
		if r.upsertErr != nil {
			return metadatamodel.LocalizedText{}, r.upsertErr
		}
		return metadatamodel.LocalizedText{}, errors.New("upsert failed")
	}
	return metadatamodel.LocalizedText{WorkspaceID: workspaceID, EntityType: req.EntityType, EntityKey: req.EntityKey, Property: req.Property, Locale: req.Locale, Text: req.Text}, nil
}
...
func (*localizedTextHandlerDictionary) Items(context.Context, metadatarepository.MetadataRepository, string, string, principalmodel.Principal) (metadatamodel.DictionaryItemsResult, bool, error) {
	return metadatamodel.DictionaryItemsResult{}, false, nil
}
