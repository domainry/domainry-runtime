...
func localizedTextCSV(values []metadatamodel.LocalizedText) []byte {
...
func localizedTextXLSX(values []metadatamodel.LocalizedText) []byte {
	var buf bytes.Buffer
...
func localizedTextWorksheetXML(values []metadatamodel.LocalizedText) string {
...
func localizedTextUpsertRequestsFromCSV(reader io.Reader) ([]metadatamodel.LocalizedTextUpsertRequest, error) {
	...
	requests := make([]metadatamodel.LocalizedTextUpsertRequest, 0, len(rows)-1)
	for rowIndex, row := range rows[1:] {
		...
		req := metadatamodel.LocalizedTextUpsertRequest{
...
func validateLocalizedTextUpsertRequests(requests []metadatamodel.LocalizedTextUpsertRequest, allowed map[string]bool) error {
	if len(allowed) == 0 {
