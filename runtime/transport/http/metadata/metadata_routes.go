...
func (h *MetadataHandler) RegisterRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("GET /tenant-admin/metadata/definitions/{resourceType}", authenticated(h.listMetadataDefinitions))
	mux.HandleFunc("GET /tenant-admin/metadata/definitions/{resourceType}/{resourceKey}", authenticated(h.getMetadataDefinition))
	mux.HandleFunc("POST /tenant-admin/metadata/definitions/{resourceType}/{resourceKey}/validate", authenticated(h.validateMetadataDefinition))
	mux.HandleFunc("GET /tenant-admin/metadata/definitions/{resourceType}/{resourceKey}/versions", authenticated(h.listMetadataDefinitionVersions))
	mux.HandleFunc("GET /tenant-admin/metadata/definitions/{resourceType}/{resourceKey}/diff", authenticated(h.diffMetadataDefinitionVersions))
