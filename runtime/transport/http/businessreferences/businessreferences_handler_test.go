...
func businessReferenceAdmin() principalmodel.Principal {
	return accessfixture.Attach(principalmodel.Principal{Principal: identitysdk.Principal{Known: true, UserID: "admin", WorkspaceID: "default"}}, accessfixture.Bundle{Permissions: []string{"workspace.admin", "*"}})
}

func TestBusinessReferenceGraphAndImpact(t *testing.T) {
	handler := businessReferenceTestHandler(businessReferenceAdmin())
	request := httptest.NewRequest(http.MethodGet, "/domain-reference-graph", nil)
	...
	response = httptest.NewRecorder()
	handler.businessReferenceImpact(response, httptest.NewRequest(http.MethodGet, "/domain-references//", nil))
	...
	request = httptest.NewRequest(http.MethodGet, "/domain-references/object/", nil)
	...
	request = httptest.NewRequest(http.MethodGet, "/domain-references/object/customer", nil)
	request.SetPathValue("resourceType", "object")
...
func TestBusinessReferenceErrorsAndRoutes(t *testing.T) {
	...
	response := httptest.NewRecorder()
	handler.businessReferenceGraph(response, httptest.NewRequest(http.MethodGet, "/domain-reference-graph", nil))
	if response.Code != http.StatusForbidden {
		t.Fatalf("graph error status=%d", response.Code)
	}
	response = httptest.NewRecorder()
	handler.businessReferenceImpact(response, httptest.NewRequest(http.MethodGet, "/domain-references/object/customer", nil))
	if response.Code != http.StatusForbidden {
	...
	response = httptest.NewRecorder()
	mux.ServeHTTP(response, httptest.NewRequest(http.MethodGet, "/domain-reference-graph", nil))
	if response.Code != http.StatusOK {
