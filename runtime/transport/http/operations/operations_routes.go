...
func (h *OperationsHandler) RegisterRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("POST /operations/dead-letters/bulk/dry-run", h.authenticated(h.dryRunBulkDeadLetters))
	mux.HandleFunc("POST /operations/dead-letters/bulk/apply", h.authenticated(h.applyBulkDeadLetters))
	...
	mux.HandleFunc("POST /operations/break-glass-grants", h.authenticated(h.enableBreakGlass))
	mux.HandleFunc("GET /operations/break-glass-grants", h.authenticated(h.listBreakGlass))
	mux.HandleFunc("POST /operations/break-glass-grants/{grantID}/revoke", h.authenticated(h.disableBreakGlass))
