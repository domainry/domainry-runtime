...
func TestOpsWorkflowProcessListAndSafeDetailHandlers(t *testing.T) {
	...
	writer, request := workflowHTTPRequest(http.MethodGet, "/workflow/operations/processes?status=failed&status=configuration_error&resource_id=process-1&limit=25", "", nil)
	...
	writer, request = workflowHTTPRequest(http.MethodGet, "/workflow/operations/processes/process-1", "", map[string]string{"processID": " process-1 "})
	...
	writer, request = workflowHTTPRequest(http.MethodPost, "/workflow/operations/processes/process-1/resolve", `{"note":"configuration repaired"}`, map[string]string{"processID": "process-1"})
