...
func TestMemoryHTTPMetricsCollectorIsConcurrentAndBoundsSeries(t *testing.T) {
	...
	for index := 0; index < 100; index++ {
		...
		go func(index int) {
			...
			collector.Observe("GET", "/records/{objectKey}", status, 10*time.Millisecond)
		}(index)
	}
	group.Wait()
	collector.Begin()
	...
	if snapshot := collector.Snapshot(); len(snapshot) != 2 || snapshot[0].Route != "/records/{objectKey}" || len(snapshot[0].DurationBuckets) != len(httpDurationBuckets) {
