...
func TestListenerRouteGroupUsesSharedRateLimiterAndFailsClosed(t *testing.T) {
	...
	first := httptest.NewRecorder()
	firstRouter.withListenerRouteGroupPolicy(ListenerRouteGroupPublic, next).ServeHTTP(first, httptest.NewRequest(http.MethodGet, "/objects/customer/records", nil))
	second := httptest.NewRecorder()
	secondRouter.withListenerRouteGroupPolicy(ListenerRouteGroupPublic, next).ServeHTTP(second, httptest.NewRequest(http.MethodGet, "/objects/customer/records", nil))
	...
	failing.withListenerRouteGroupPolicy(ListenerRouteGroupPublic, next).ServeHTTP(unavailable, httptest.NewRequest(http.MethodGet, "/objects/customer/records", nil))
