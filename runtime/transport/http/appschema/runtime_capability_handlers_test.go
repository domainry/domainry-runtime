...
func (d *runtimeMetadataDictionary) Items(_ context.Context, _ appschemarepository.ApplicationSchemaRepository, key, locale string, _ principalmodel.Principal) (appschemamodel.DictionaryItemsResult, bool, error) {
...
func newRuntimeCapabilityHandler(t *testing.T, repository *definitionLifecycleRepository, dictionary *runtimeMetadataDictionary) (*ApplicationSchemaHandler, *localizedTextHandlerCapture, *int) {
	t.Helper()
	runtimeCatalog := appschemaapplication.NewApplicationSchemaApplicationService(appschemaapplication.ApplicationSchemaDependencies{
...
func TestRuntimeCapabilityHandlersSuccessAndServiceErrors(t *testing.T) {
	repository := &definitionLifecycleRepository{migrationSteps: []appschemamodel.ApplicationSchemaMigrationStep{{ObjectKey: "customer", Operation: "create_table"}}}
	dictionary := &runtimeMetadataDictionary{result: appschemamodel.DictionaryItemsResult{DictionaryKey: "status"}, found: true}
	...
	runtimeCatalogWithError := appschemaapplication.NewApplicationSchemaApplicationService(appschemaapplication.ApplicationSchemaDependencies{
		Repository: repository,
		Runtime:    localizedTextHandlerRuntime{snapshot: appschemamodel.ApplicationSchemaSnapshot{Objects: []definitionmodel.ObjectSchema{{Key: "customer"}}}},
