...
func TestMetadataAuthoringHeadersBindBuilderIdempotencyAndSchemaHash(t *testing.T) {
	errorCode := ""
	handler := &ApplicationSchemaHandler{writeError: func(w http.ResponseWriter, _ *http.Request, status int, code string, _ ...string) {
		errorCode = code
	...
	body := appschemamodel.ApplicationDefinitionUpsertRequest{}
...
func TestMetadataAuthoringHeadersRepresentCreateWithExplicitEmptyResourceHash(t *testing.T) {
	handler := &ApplicationSchemaHandler{writeError: func(w http.ResponseWriter, _ *http.Request, status int, _ string, _ ...string) { w.WriteHeader(status) }}
	request := httptest.NewRequest(http.MethodPut, "/metadata/definitions/object/order", nil)
	request.Header.Set("Builder-Task-ID", "task-1")
	request.Header.Set("Idempotency-Key", "create-order")
...
func TestMetadataAuthoringHeadersCoverOptionalAndExistingSourceFields(t *testing.T) {
	handler := &ApplicationSchemaHandler{
	...
	request := httptest.NewRequest(http.MethodPut, "/metadata/definitions/object/order", nil)
	bodyHash := "schema-1"
	body := appschemamodel.ApplicationDefinitionUpsertRequest{ExpectedSchemaHash: &bodyHash}
