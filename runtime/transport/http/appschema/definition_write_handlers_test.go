...
func newDefinitionWriteHandler(t *testing.T, repository *definitionLifecycleRepository) (*ApplicationSchemaHandler, *localizedTextHandlerCapture, *definitionLifecycleAuditRepository) {
	t.Helper()
	...
	definitions := appschemaapplication.NewApplicationSchemaApplicationService(appschemaapplication.ApplicationSchemaDependencies{
		Repository: repository,
		Runtime:    localizedTextHandlerRuntime{snapshot: appschemamodel.ApplicationSchemaSnapshot{Name: "Runtime", SchemaHash: "schema-hash", Objects: []definitionmodel.ObjectSchema{{Key: "customer"}}}},
...
func TestListApplicationDefinitionsDefaultsWorkspaceAndRejectsFailures(t *testing.T) {
	repository := &definitionLifecycleRepository{definitions: []appschemamodel.ApplicationDefinition{{ResourceType: "view", ResourceKey: "customer-list"}}}
	handler, capture, _ := newDefinitionWriteHandler(t, repository)

	response := httptest.NewRecorder()
	handler.listApplicationDefinitions(response, definitionLifecycleRequest(http.MethodGet, "/metadata/definitions/view", "", " view ", ""))
	...
	handler.listApplicationDefinitions(crossWorkspaceResponse, definitionLifecycleRequest(http.MethodGet, "/?workspace_id=workspace-b", "", "view", ""))
	...
	handler.listApplicationDefinitions(errorResponse, definitionLifecycleRequest(http.MethodGet, "/?workspace_id=workspace-a", "", "view", ""))
...
func TestValidateApplicationDefinitionHandlerSuccessDecodeAndValidationError(t *testing.T) {
	...
	response := httptest.NewRecorder()
	handler.validateApplicationDefinition(response, request)
	...
	handler.validateApplicationDefinition(badJSONResponse, definitionLifecycleRequest(http.MethodPost, "/", "{", "preference", "theme"))
	if badJSONResponse.Code != http.StatusBadRequest {
		t.Fatalf("bad JSON status = %d", badJSONResponse.Code)
	}

	invalidResponse := httptest.NewRecorder()
	handler.validateApplicationDefinition(invalidResponse, definitionLifecycleRequest(http.MethodPost, "/", `{"payload":{}}`, "unsupported", "key"))
	...
	handler.validateApplicationDefinition(directResponse, directRequest)
	...
	handler.writeMetadataAuthoringValidationResult(emptyFailureResponse, directRequest, appschemamodel.ApplicationDefinitionValidationResult{Valid: false})
	...
	handler.validateApplicationDefinition(unauthorizedResponse, definitionLifecycleRequest(http.MethodPost, "/", `{"payload":{"key":"theme"}}`, "preference", "theme"))
