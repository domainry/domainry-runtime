...
type definitionLifecycleRepository struct {
	...
	versionsErr        error
	rolledBack         appschemamodel.ApplicationDefinition
	published          appschemamodel.ApplicationDefinition
	publishErr         error
	completeRefreshErr error
	migrationSteps     []appschemamodel.ApplicationSchemaMigrationStep
	migrationErr       error
	disableErr         error
	loadErr            error
	syncErr            error
...
func (r *definitionLifecycleRepository) GetDefinition(context.Context, principalmodel.SystemScope, string, string) (appschemamodel.ApplicationDefinition, bool, error) {
	return r.definition, r.found, r.getErr
}

func (r *definitionLifecycleRepository) ListDefinitionVersions(context.Context, principalmodel.SystemScope, string, string) ([]appschemamodel.ApplicationDefinitionVersion, error) {
	return append([]appschemamodel.ApplicationDefinitionVersion(nil), r.versions...), r.versionsErr
}

func (r *definitionLifecycleRepository) ListDefinitions(context.Context, principalmodel.SystemScope, string) ([]appschemamodel.ApplicationDefinition, error) {
	return append([]appschemamodel.ApplicationDefinition(nil), r.definitions...), r.definitionsErr
}

func (r *definitionLifecycleRepository) RollbackDefinition(context.Context, principalmodel.SystemScope, string, string, appschemamodel.ApplicationDefinitionRollbackRequest, auditmodel.AuditEvent) (appschemamodel.ApplicationDefinition, error) {
	return r.rolledBack, nil
}

func (r *definitionLifecycleRepository) PublishDefinition(context.Context, principalmodel.SystemScope, string, string, appschemamodel.ApplicationDefinitionUpsertRequest, auditmodel.AuditEvent) (appschemamodel.ApplicationDefinition, error) {
...
func (r *definitionLifecycleRepository) MigrationPlan(context.Context, principalmodel.SystemScope, manifestmodel.ManifestSchema) ([]appschemamodel.ApplicationSchemaMigrationStep, error) {
	return append([]appschemamodel.ApplicationSchemaMigrationStep(nil), r.migrationSteps...), r.migrationErr
}
...
func newDefinitionLifecycleHandler(t *testing.T, repository *definitionLifecycleRepository, references definitionLifecycleReferences, operations *definitionLifecycleOperations) (*ApplicationSchemaHandler, *localizedTextHandlerCapture, *definitionLifecycleAuditRepository) {
	t.Helper()
	...
	service := appschemaapplication.NewApplicationSchemaApplicationService(appschemaapplication.ApplicationSchemaDependencies{
		Repository:    repository,
		Runtime:       localizedTextHandlerRuntime{snapshot: appschemamodel.ApplicationSchemaSnapshot{Name: "Runtime", SchemaHash: "schema-hash"}},
...
func TestDefinitionLifecycleGetAndListVersions(t *testing.T) {
	repository := &definitionLifecycleRepository{
		definition: appschemamodel.ApplicationDefinition{ResourceType: "view", ResourceKey: "customer-list", SchemaVersion: "2", Payload: json.RawMessage(`{"key":"customer-list"}`)},
		found:      true,
		versions: []appschemamodel.ApplicationDefinitionVersion{
	...
	handler.getApplicationDefinition(getResponse, definitionLifecycleRequest(http.MethodGet, "/", "", " view ", " customer-list "))
	...
	handler.listApplicationDefinitionVersions(listResponse, definitionLifecycleRequest(http.MethodGet, "/", "", " view ", " customer-list "))
	...
	handler.getApplicationDefinition(notFoundResponse, definitionLifecycleRequest(http.MethodGet, "/", "", "view", "missing"))
	...
	handler.getApplicationDefinition(errorResponse, definitionLifecycleRequest(http.MethodGet, "/", "", "view", "customer-list"))
	...
	for _, handle := range []func(http.ResponseWriter, *http.Request){handler.listApplicationDefinitionVersions, handler.diffApplicationDefinitionVersions} {
		capture.serviceErr = nil
...
func TestDiffApplicationDefinitionVersionsUsesLatestVersionWhenToIsOmitted(t *testing.T) {
	repository := &definitionLifecycleRepository{versions: []appschemamodel.ApplicationDefinitionVersion{
		{SchemaVersion: "2", Payload: json.RawMessage(`{"name":"new"}`)},
		{SchemaVersion: "1", Payload: json.RawMessage(`{"name":"old"}`)},
	}}
	...
	response := httptest.NewRecorder()
	handler.diffApplicationDefinitionVersions(response, definitionLifecycleRequest(http.MethodGet, "/?from=%201%20", "", " view ", " customer-list "))
	...
	handler.diffApplicationDefinitionVersions(explicitResponse, definitionLifecycleRequest(http.MethodGet, "/?from=2&to=1", "", "view", "customer-list"))
	...
	handler.diffApplicationDefinitionVersions(emptyResponse, definitionLifecycleRequest(http.MethodGet, "/", "", "view", "customer-list"))
