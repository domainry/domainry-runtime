...
func TestOpsMetadataDiagnosticsHandlerSuccessAndServiceError(t *testing.T) {
	...
	service := appschemaapplication.NewApplicationSchemaApplicationService(appschemaapplication.ApplicationSchemaDependencies{
		...
		Runtime: localizedTextHandlerRuntime{snapshot: appschemamodel.ApplicationSchemaSnapshot{
			SchemaHash: "schema-hash", SnapshotVersion: "snapshot-1", TemplateVersion: "1",
			Objects: []definitionmodel.ObjectSchema{{Key: "customer"}},
		}},
	})
	principal := accessfixture.Attach(principalmodel.Principal{Principal: identitysdk.Principal{Known: true, WorkspaceID: "workspace-a"}}, accessfixture.Bundle{Permissions: []string{appschemaapplication.PermissionMetadataOpsRead}})
