...
func (h *ApplicationSchemaHandler) getApplicationDefinition(w http.ResponseWriter, r *http.Request) {
	...
	definition, ok, err := h.definitions.GetApplicationDefinition(r.Context(), resourceType, resourceKey, h.principal(r))
	if err != nil {
...
func (h *ApplicationSchemaHandler) listApplicationDefinitionVersions(w http.ResponseWriter, r *http.Request) {
	versions, err := h.definitions.ListApplicationDefinitionVersions(r.Context(), strings.TrimSpace(r.PathValue("resourceType")), strings.TrimSpace(r.PathValue("resourceKey")), h.principal(r))
	if err != nil {
		h.writeServiceError(w, r, err)
		return
	}
...
func (h *ApplicationSchemaHandler) diffApplicationDefinitionVersions(w http.ResponseWriter, r *http.Request) {
	resourceType, resourceKey := strings.TrimSpace(r.PathValue("resourceType")), strings.TrimSpace(r.PathValue("resourceKey"))
	fromVersion, toVersion := strings.TrimSpace(r.URL.Query().Get("from")), strings.TrimSpace(r.URL.Query().Get("to"))
	versions, err := h.definitions.ListApplicationDefinitionVersions(r.Context(), resourceType, resourceKey, h.principal(r))
	if err != nil {
		h.writeServiceError(w, r, err)
		return
	}
