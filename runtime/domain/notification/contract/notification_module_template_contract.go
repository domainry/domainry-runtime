...
func ModuleTemplate(value notificationmodel.NotificationTemplate) sourcetemplate.Template {
	variables := make([]sourcetemplate.Variable, len(value.Variables))
	for i, v := range value.Variables {
		variables[i] = sourcetemplate.Variable{Key: v.Key, Type: v.Type, Required: v.Required}
	}
	fallbacks := make([]sourcetemplate.Fallback, len(value.Fallbacks))
	for i, v := range value.Fallbacks {
		fallbacks[i] = sourcetemplate.Fallback{TemplateKey: v.TemplateKey, ConnectorKey: v.ConnectorKey, ConnectionKey: v.ConnectionKey, Operation: v.Operation}
	}
	locales := make(map[string]sourcetemplate.Content, len(value.Locales))
	for key, content := range value.Locales {
		locales[key] = moduleContent(content)
	}
	return sourcetemplate.Template{Key: value.Key, Name: value.Name, Channel: value.Channel, Provider: value.Provider, Status: value.Status, Version: value.Version, DefaultLocale: value.DefaultLocale, Variables: variables, Fallbacks: fallbacks, Locales: locales, ContentHash: value.ContentHash}
}
...
func moduleContent(value notificationmodel.NotificationTemplateContent) sourcetemplate.Content {
	facts := make([]sourcetemplate.Fact, len(value.Facts))
	for i, v := range value.Facts {
		facts[i] = sourcetemplate.Fact{Key: v.Key, Value: v.Value}
	}
	actions := make([]sourcetemplate.Action, len(value.Actions))
	for i, v := range value.Actions {
		actions[i] = sourcetemplate.Action{Label: v.Label, URL: v.URL, Style: v.Style}
	}
	var provider *sourcetemplate.ProviderTemplate
	if value.ProviderTemplate != nil {
		components := make([]sourcetemplate.ProviderTemplateComponent, len(value.ProviderTemplate.Components))
		for i, v := range value.ProviderTemplate.Components {
			components[i] = sourcetemplate.ProviderTemplateComponent{Type: v.Type, SubType: v.SubType, Index: v.Index, Parameters: append([]string(nil), v.Parameters...)}
		}
		provider = &sourcetemplate.ProviderTemplate{Name: value.ProviderTemplate.Name, Language: value.ProviderTemplate.Language, Components: components}
	}
	return sourcetemplate.Content{Subject: value.Subject, Title: value.Title, Text: value.Text, HTML: value.HTML, Markdown: value.Markdown, Facts: facts, Actions: actions, ProviderTemplate: provider}
}

func planeContent(value sourcetemplate.Content) notificationmodel.NotificationTemplateContent {
