...
func TestRuntimeAuthoringDeliveryRequiresBoundCompleteScenarioEvidence(t *testing.T) {
	...
	replayOne, replayTwo := step("replay-one", "/records/order", 200, strings.Repeat("b", 64)), step("replay-two", "/records/order", 200, strings.Repeat("b", 64))
	...
	evidence := changeplanmodel.RuntimeAuthoringDeliveryEvidence{
		...
		Scenarios: []changeplanmodel.RuntimeAuthoringScenarioEvidence{{
			...
			Steps: []changeplanmodel.RuntimeAuthoringScenarioStepEvidence{
				step("success", "/records/order/items/order-1", 200, strings.Repeat("c", 64)),
				step("denied", "/records/order/items/order-1", 403, strings.Repeat("d", 64)),
				step("precondition", "/records/order/items/order-1/actions/complete", 422, strings.Repeat("e", 64)),
				replayOne, replayTwo,
