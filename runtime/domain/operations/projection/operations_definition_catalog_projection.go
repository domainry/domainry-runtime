...
var operationsDefinitionCatalog = []operationsmodel.OperationsDefinition{
	operationsDefinition("scheduler.job.run", "scheduler", "scheduler_definition", false, []string{"runtime.scheduler.run_ops_scheduler_job"}, "published definition exists and target readiness passes"),
	operationsDefinition("scheduler.definition.reschedule", "scheduler", "scheduler_definition", false, []string{"runtime.scheduler.reschedule_ops_scheduler_definition"}, "published definition exists and next_run_at is valid"),
	operationsDefinition("scheduler.run.retry", "scheduler", "scheduler_run", false, []string{"runtime.scheduler.retry_ops_scheduler_run"}, "run is failed, retryable, and below max attempts"),
	operationsDefinition("scheduler.run.cancel", "scheduler", "scheduler_run", false, []string{"runtime.scheduler.cancel_ops_scheduler_run"}, "run is cancellable and caller observes current status"),
	operationsDefinition("scheduler.dead_letter.resolve", "scheduler", "scheduler_dead_letter", false, []string{"runtime.scheduler.resolve_ops_scheduler_dead_letter"}, "dead letter is unresolved and resolution note is present"),
	operationsDefinition("scheduler.dead_letter.requeue", "scheduler", "scheduler_dead_letter", false, []string{"runtime.scheduler.requeue_ops_scheduler_dead_letter"}, "dead letter is unresolved and its target is currently dispatchable"),
	...
	operationsDefinition("workflow.execution.retry", "workflow", "workflow_execution", false, []string{"ops.workflow.retry"}, "execution is failed or dead_letter and attempt budget remains"),
	operationsDefinition("workflow.execution.resolve", "workflow", "workflow_execution", false, []string{"ops.workflow.resolve"}, "execution is dead_letter"),
	...
	operationsSystemDefinition("database.retirement.execute", "operations", "database_object", true, []string{"runtime.operations.execute_database_retirement"}, "observation, comparison, backup, restore drill, maintenance, drain, approval, and quarantine evidence pass"),
