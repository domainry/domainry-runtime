...
func TestMetadataValidateViewDefinitionUsesLiveObjectFields(t *testing.T) {
	...
	if normalized, err := MetadataValidateViewDefinition("order_list", valid, objects); err != nil || len(normalized) == 0 {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			if _, err := MetadataValidateViewDefinition("order_list", json.RawMessage(test.payload), objects); apperror.CodeOf(err) != test.code {
