...
func TestMetadataValidateObjectDefinitionOwnsSmallObjectShell(t *testing.T) {
	normalized, err := MetadataValidateObjectDefinition("order", json.RawMessage(`{"key":" order ","name":" Order "}`))
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			_, err := MetadataValidateObjectDefinition("order", json.RawMessage(test.payload))
...
func TestMetadataValidateObjectDefinitionPreservesProfileUX(t *testing.T) {
	normalized, err := MetadataValidateObjectDefinition("technician_profile", json.RawMessage(`{"key":"technician_profile","name":"Technician profile","ux":{"kind":"identity_profile_extension","config":{"identity_relation_field":"identity_user"},"display":{"title_field":"certificate_no"}}}`))
...
func TestMetadataValidateObjectDefinitionNormalizesLifecyclePolicy(t *testing.T) {
	normalized, err := MetadataValidateObjectDefinition("ledger_entry", json.RawMessage(`{"key":"ledger_entry","name":"Ledger entry","lifecycle_policy":{"mode":" immutable_after_state ","state_field":" status ","immutable_states":[" confirmed ","reversed"]}}`))
	...
	for _, payload := range []string{
		...
		_, err := MetadataValidateObjectDefinition("ledger_entry", json.RawMessage(payload))
...
func TestMetadataValidateObjectDefinitionNormalizesLedgerPolicy(t *testing.T) {
	...
	for _, payload := range []string{
		...
		_, err := MetadataValidateObjectDefinition("financial_entry", json.RawMessage(payload))
...
func TestMetadataValidateObjectDefinitionNormalizesExportAssurancePolicy(t *testing.T) {
	normalized, err := MetadataValidateObjectDefinition("customer", json.RawMessage(`{"key":"customer","name":"Customer","export_assurance_policy":{"required_methods":[" otp "," recent_reauth "],"recent_reauth_max_age_seconds":300}}`))
	...
	for _, payload := range []string{
		...
		if _, err := MetadataValidateObjectDefinition("customer", json.RawMessage(payload)); apperror.CodeOf(err) != "backend.metadata.object_export_assurance_policy_invalid" {
			t.Fatalf("payload=%s err=%v", payload, err)
		}
	}
}
...
func TestMetadataValidateObjectDefinitionRemainingConditions(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			normalized, err := MetadataValidateObjectDefinition(test.resource, json.RawMessage(test.payload))
