...
func TestConnectorValidationEdges(t *testing.T) {
	...
	if err := MetadataValidateConnectorDefinition(valid); err != nil {
	...
	if issues := MetadataValidateConnectorDefinitionIssues(declaredSecret); len(issues) != 0 {
	...
	if issues := MetadataValidateConnectorDefinitionIssues(reserve); len(issues) != 0 {
		t.Fatalf("complete reserve: %#v", issues)
	}
	reserve.Operations[0].CompensationOperation = ""
	if !hasMetadataIssue(MetadataValidateConnectorDefinitionIssues(reserve), "backend.integration.connector.reserve_contract_incomplete") {
...
func TestFieldMutationNormalizationEdges(t *testing.T) {
	...
	assertCode := func(t *testing.T, req metadatamodel.MetadataDefinitionUpsertRequest, code string, records []recordmodel.Record) {
		t.Helper()
		_, err := MetadataNormalizeFieldMutation(req, objects, []string{"text", "relation"}, records, len(records))
		var appErr *apperror.AppError
		if !errors.As(err, &appErr) || appErr.Code != code {
			t.Fatalf("want %s, got %#v", code, err)
		}
	}
	assertCode(t, metadatamodel.MetadataDefinitionUpsertRequest{Payload: json.RawMessage(`{`)}, "backend.metadata.field_definition_invalid", nil)
	...
	if _, err := MetadataNormalizeFieldMutation(configuredObject, objects, []string{"text"}, nil, 0); err != nil {
	...
	base.Required = false
	if _, err := MetadataNormalizeFieldMutation(request(base), objects, []string{"relation"}, nil, 0); err != nil {
	...
	normalized, err := MetadataNormalizeFieldMutation(request(base), objects, []string{"relation"}, nil, 0)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := MetadataNormalizeFieldMutation(request(text), objects, []string{"text"}, nil, 0); err != nil {
	...
	text.Default = "x"
	if _, err := MetadataNormalizeFieldMutation(request(text), objects, []string{"text"}, []recordmodel.Record{{Data: map[string]any{}}}, 1); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := MetadataNormalizeFieldMutation(request(existingOptional), objects, []string{"text"}, []recordmodel.Record{{Data: map[string]any{"optional": "Ada"}}}, 1); err != nil {
	...
	if _, err := MetadataNormalizeFieldMutation(configObject, objects, []string{"text"}, nil, 0); err != nil {
		t.Fatalf("config object key: %v", err)
	}
	...
	if _, err := MetadataNormalizeFieldMutation(identityRelation, objects, []string{"relation"}, nil, 0); err != nil {
	...
	if _, err := MetadataNormalizeFieldMutation(departmentRelation, objects, []string{"relation"}, nil, 0); err != nil {
	...
	if _, err := MetadataNormalizeFieldMutation(request(many), objects, []string{"relation"}, nil, 0); err != nil {
	...
	if _, err := MetadataNormalizeFieldMutation(request(defaultValue), objects, []string{"text"}, []recordmodel.Record{{Data: map[string]any{}}}, 1); err != nil {
		t.Fatalf("default value: %v", err)
	}
	existingRequired := definitionmodel.FieldSchema{Key: "owner", Type: "text", Required: true}
	if _, err := MetadataNormalizeFieldMutation(request(existingRequired), objects, []string{"text"}, []recordmodel.Record{{Data: map[string]any{}}}, 1); err != nil {
		t.Fatalf("existing required: %v", err)
	}
	optional := definitionmodel.FieldSchema{Key: "optional", Type: "text"}
	if _, err := MetadataNormalizeFieldMutation(request(optional), objects, []string{"text"}, []recordmodel.Record{{Data: map[string]any{}}}, 1); err != nil {
		t.Fatalf("optional: %v", err)
	}
}

func TestDefinitionValidationUtilityEdges(t *testing.T) {
	if MetadataFirstDefinitionIssueError(nil) != nil {
		t.Fatal("empty issues should pass")
	}
	if err := MetadataFirstDefinitionIssueError([]metadatamodel.MetadataDefinitionValidationIssue{{ErrorCode: "empty-path"}}); err == nil {
	...
	if MetadataNormalizedDefinitionValue(nil) != "" || MetadataNormalizedDefinitionValue(" x ") != "x" || MetadataNormalizedDefinitionValue((*int)(nil)) != "" {
		t.Fatal("normalization mismatch")
	}
	issue := NewMetadataDefinitionValidationIssue("backend.action.unknown", "", "step", "op", nil)
	...
	for code, want := range map[string]string{
		...
		if got := MetadataDefinitionValidationErrorFieldPath(code, nil); got != want {
	...
	for code, capability := range map[string]string{
		...
		if got := NewMetadataDefinitionValidationIssue(code, "field", "", "", nil).CapabilityKey; got != capability {
			t.Fatalf("%s capability=%q", code, got)
		}
	}
	if got := MetadataDefinitionValidationErrorFieldPath("other", map[string]string{"field": " custom "}); got != "custom" {
...
func TestDefinitionValidationRequestEdges(t *testing.T) {
	...
	cases := []struct {
		...
		validator                       MetadataDefinitionPayloadValidator
		code                            string
	}{
		...
		{"plain error", "action", "key", json.RawMessage(`{}`), func(context.Context, string, string, json.RawMessage) (json.RawMessage, []metadatamodel.MetadataDefinitionValidationIssue, error) {
			return nil, nil, errors.New("down")
		}, "backend.internal"},
		{"issues", "action", "key", json.RawMessage(`{}`), func(context.Context, string, string, json.RawMessage) (json.RawMessage, []metadatamodel.MetadataDefinitionValidationIssue, error) {
			return nil, []metadatamodel.MetadataDefinitionValidationIssue{{ErrorCode: "owner.issue"}}, nil
		}, "owner.issue"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			result, err := MetadataValidateDefinitionRequest(t.Context(), tc.resourceType, tc.resourceKey, tc.payload, admin, tc.validator)
	...
	result := definitionValidationFailure(metadatamodel.MetadataDefinitionValidationResult{}, badRequest("custom", "field", ""))
...
func TestDictionaryValidationJSONEdges(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			err := MetadataValidateDictionaryDefinition("dict", json.RawMessage(tc.payload))
			var appErr *apperror.AppError
	...
	if err := MetadataValidateDictionaryDefinition(" dict ", valid); err != nil {
...
func TestDefinitionRequestFailureEdges(t *testing.T) {
	admin := accessfixture.Attach(principalmodel.Principal{Principal: identitysdk.Principal{Known: true}}, accessfixture.Bundle{Permissions: []string{"workspace.admin"}})
	if _, err := MetadataValidateDefinitionRequest(t.Context(), "action", "x", json.RawMessage(`{}`), principalmodel.Principal{Principal: identitysdk.Principal{Known: true}}, nil); err == nil {
		t.Fatal("known non-admin must be forbidden")
	}
	validator := func(context.Context, string, string, json.RawMessage) (json.RawMessage, []metadatamodel.MetadataDefinitionValidationIssue, error) {
		return nil, []metadatamodel.MetadataDefinitionValidationIssue{{ErrorCode: "owned"}}, nil
	}
	for _, tc := range []struct {
		name, resourceType, resourceKey string
		payload                         json.RawMessage
		validate                        MetadataDefinitionPayloadValidator
		code                            string
	}{
		...
		t.Run(tc.name, func(t *testing.T) {
			result, err := MetadataValidateDefinitionRequest(t.Context(), tc.resourceType, tc.resourceKey, tc.payload, admin, tc.validate)
			if err != nil || result.Valid || len(result.Errors) != 1 || result.Errors[0].ErrorCode != tc.code {
				t.Fatalf("result=%#v err=%v", result, err)
			}
		})
	}
	result, err := MetadataValidateDefinitionRequest(t.Context(), "action", "x", json.RawMessage(`{}`), admin, validator)
	if err != nil || len(result.Errors) != 1 || result.Errors[0].ErrorCode != "owned" {
		t.Fatalf("owned issues: %#v %v", result, err)
	}
	result, err = MetadataValidateDefinitionRequest(t.Context(), "action", "x", json.RawMessage(`{}`), admin,
...
func TestReportValidationEdges(t *testing.T) {
	...
	if issues := MetadataValidateReportDefinitionContract(t.Context(), snapshot, validPermissionReport); len(issues) != 0 {
		t.Fatalf("known permission: %#v", issues)
	}
	issues := MetadataValidateReportDefinitionContract(t.Context(), snapshot, invalid)
	...
	if !hasMetadataIssue(MetadataValidateReportDefinitionContract(t.Context(), snapshot, missingSources), "backend.report.dataset_source_invalid") {
	...
	if issues := MetadataValidateReportDefinitionContract(t.Context(), snapshot, fieldCombinationReport); !hasMetadataIssue(issues, "backend.report.field_reference_invalid") {
	...
	if err := MetadataValidateReportDefinition(t.Context(), "ws", snapshot, reader, report); err != nil {
	...
	issues = MetadataValidateReportDefinitionIssues(t.Context(), "ws", snapshot, errReader, report)
	if !hasMetadataIssue(issues, "backend.report.evidence_unavailable") {
		t.Fatalf("missing unavailable: %#v", issues)
	}

	validator := reportDefinitionValidator{issues: []metadatamodel.MetadataDefinitionValidationIssue{}}
	...
	emptyIssues := MetadataValidateReportDefinitionContract(t.Context(), snapshot, reportmodel.ReportSchema{})
	...
	if issues := MetadataValidateReportDefinitionContract(t.Context(), snapshot, missingObjectEvidence); !hasMetadataIssue(issues, "backend.report.source_object_not_found") {
		t.Fatalf("missing object issues=%#v", issues)
	}
}

func hasMetadataIssue(issues []metadatamodel.MetadataDefinitionValidationIssue, code string) bool {
