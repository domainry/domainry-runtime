...
func MetadataFirstDefinitionIssueError(issues []metadatamodel.MetadataDefinitionValidationIssue) error {
	if len(issues) == 0 {
		return nil
	}
	issue := issues[0]
...
func MetadataNormalizedDefinitionValue(value any) string {
	if value == nil {
		return ""
	}
