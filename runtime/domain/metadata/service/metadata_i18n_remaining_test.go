...
func (r metadataLocaleRepository) ListLocalizedTexts(context.Context, string, metadatamodel.LocalizedTextQuery) ([]metadatamodel.LocalizedText, error) {
...
func TestMetadataSchemaLocaleProjectsAllOwnedShapes(t *testing.T) {
	snapshot := metadatamodel.MetadataSchemaSnapshot{
		...
		GuardedWrites: []metadatamodel.MetadataGuardedWriteContract{{ActionKey: "order.approve", Label: "Approve"}},
		Workflows:     []definitionmodel.WorkflowSchema{{Key: "notify", Name: "Notify"}},
		Dictionaries:  []metadatamodel.DictionarySchema{{Key: "status", Name: "Status", Description: "Status values", Items: []metadatamodel.DictionaryItemSchema{{Value: "ready", Label: "Ready", Description: "Ready description"}}}},
	...
	values := []metadatamodel.LocalizedText{{EntityType: "app", EntityKey: "app", Property: "name", Text: "Anwendung"}}
	service := NewMetadataSchemaDomainService(schemaServiceProviderStub{snapshot: snapshot}, metadataLocaleRepository{values: values})
