...
func (s *MetadataSchemaDomainService) ForPrincipalLocale(ctx context.Context, principal principalmodel.Principal, locale string) metadatamodel.MetadataSchemaSnapshot {
	...
	values, err := s.metadata.ListLocalizedTexts(ctx, principal.WorkspaceID, metadatamodel.LocalizedTextQuery{WorkspaceID: principal.WorkspaceID, Locale: locale})
...
func localizedTextLookup(values []metadatamodel.LocalizedText) map[string]string {
