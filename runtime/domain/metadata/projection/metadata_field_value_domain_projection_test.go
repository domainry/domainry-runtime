...
func TestMetadataEnrichObjectsWithFieldValueDomains(t *testing.T) {
	...
	dictionaries := []metadatamodel.DictionarySchema{{Key: "ticket_status", Items: []metadatamodel.DictionaryItemSchema{{Key: "open", Value: "open", Label: "Open", SortOrder: 1}, {Key: "closed", Value: "closed", Label: "Closed", SortOrder: 2}}}}

	enriched := MetadataEnrichObjectsWithFieldValueDomains(objects, dictionaries)
...
func TestMetadataMergeGeneratedDictionaryItemsPreservesLocalizedMetadata(t *testing.T) {
	defaults := []metadatamodel.DictionaryItemSchema{{Key: "open", Value: "open", Label: "Open", Description: "Default", Color: "green", SortOrder: 3}}
	localized := []metadatamodel.DictionaryItemSchema{{Key: "open", Label: "打开", Locale: "zh-CN"}}
	items := MetadataMergeGeneratedDictionaryItems(defaults, localized)
