...
// ApplicationSchemaLocalizedTextCoverage projects persisted localized texts and a schema
// snapshot into the owner read model. It performs no authorization or I/O.
func ApplicationSchemaLocalizedTextCoverage(
	workspaceID string,
	locale string,
	fallbackLocale string,
	snapshot appschemamodel.ApplicationSchemaSnapshot,
	values []appschemamodel.LocalizedText,
) appschemamodel.LocalizedTextCoverageResult {
	byLocale := make(map[string]appschemamodel.LocalizedText, len(values))
	...
	return appschemamodel.LocalizedTextCoverageResult{
		WorkspaceID:    workspaceID,
		Locale:         locale,
		FallbackLocale: fallbackLocale,
		Items:          items,
...
func metadataLocalizedTextExpectedItems(snapshot appschemamodel.ApplicationSchemaSnapshot) []metadataLocalizedTextExpectedItem {
...
func metadataLocalizedTextCoverageSortKey(item appschemamodel.LocalizedTextCoverageItem) string {
	missingPrefix := "1"
	if item.Missing {
		missingPrefix = "0"
	}
