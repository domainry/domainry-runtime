...
func ApplicationSchemaValidateConnectorDefinition(connector integrationmodel.ConnectorSchema) error {
	return ApplicationSchemaFirstDefinitionIssueError(ApplicationSchemaValidateConnectorDefinitionIssues(connector))
}

func ApplicationSchemaValidateConnectorDefinitionIssues(connector integrationmodel.ConnectorSchema) []appschemamodel.ApplicationDefinitionValidationIssue {
	...
	declaredSecretRefs := ApplicationSchemaStringSet(connector.SecretRefs)
...
func validateConnectorOperationContract(operation integrationmodel.ConnectorOperationSchema, operationKey, prefix string) []appschemamodel.ApplicationDefinitionValidationIssue {
	issues := make([]appschemamodel.ApplicationDefinitionValidationIssue, 0)
...
func validateConnectorProtocolFieldIssues(operationKey, prefix, direction string, fields []definitionmodel.FieldSchema) []appschemamodel.ApplicationDefinitionValidationIssue {
	issues := make([]appschemamodel.ApplicationDefinitionValidationIssue, 0)
	seen := map[string]bool{}
	allowed := ApplicationSchemaStringSet(connectorProtocolFieldTypes)
...
func connectorEnumIssue(code, path, operationKey, actual string, allowed []string, legacyKey string) appschemamodel.ApplicationDefinitionValidationIssue {
...
func connectorIssue(code, path, operationKey string, params map[string]string) appschemamodel.ApplicationDefinitionValidationIssue {
	if params == nil {
		params = map[string]string{}
	}
	params["field"] = path
	return NewApplicationDefinitionValidationIssue(code, path, "", operationKey, params)
}
