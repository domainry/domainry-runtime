...
func objectSQLBoundsSnapshot() appschemamodel.ApplicationSchemaSnapshot {
	return appschemamodel.ApplicationSchemaSnapshot{Objects: []definitionmodel.ObjectSchema{{
		Key: "repair_order",
...
func objectSQLBoundsIssueParams(issues []appschemamodel.ApplicationDefinitionValidationIssue, code string) (map[string]string, bool) {
...
func TestObjectSQLContractRequiresExplicitOrderByAndLimit(t *testing.T) {
	...
	issues := ApplicationSchemaValidateReportDefinitionContract(t.Context(), objectSQLBoundsSnapshot(), report)
...
func TestObjectSQLContractGroupedPlanNeedsOnlyExplicitLimit(t *testing.T) {
	...
	if issues := ApplicationSchemaValidateReportDefinitionContract(t.Context(), objectSQLBoundsSnapshot(), grouped); len(issues) != 0 {
	...
	issues := ApplicationSchemaValidateReportDefinitionContract(t.Context(), objectSQLBoundsSnapshot(), truncating)
...
func TestObjectSQLRuntimeIssuesPathDoesNotRequireExplicitBounds(t *testing.T) {
	...
	issues := ApplicationSchemaValidateReportDefinitionIssues(t.Context(), "workspace-a", objectSQLBoundsSnapshot(), reportRecordRepositoryStub{}, report)
	for _, issue := range issues {
