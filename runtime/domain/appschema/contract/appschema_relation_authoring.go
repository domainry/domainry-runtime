...
func ApplicationSchemaRelationAuthoringCapability() capabilitycontract.CapabilityAuthoringDefinition {
	...
	return capabilitycontract.CapabilityAuthoringDefinition{
		...
		Permissions: []string{"runtime.appschema.validate_application_definition"}, ValidationEndpoint: "POST /tenant-admin/metadata/definitions/field/{resourceKey}/validate",
		...
		Examples: []capabilitycontract.CapabilityAuthoringExample{
			{Name: "minimal_valid", Value: map[string]any{"object_key": "order", "expected_schema_hash": "$instance.schema_hash", "payload": map[string]any{"name": "Customer", "type": "relation", "config": map[string]any{"target": "customer"}}}},
			{Name: "representative", Value: map[string]any{"object_key": "order", "expected_schema_hash": "$instance.schema_hash", "payload": map[string]any{"name": "Customer", "type": "relation", "required": true, "config": map[string]any{"target": "customer", "cardinality": "many_to_one", "on_delete": "restrict", "inverse_name": "orders", "indexed": true}}}},
			{Name: "invalid_with_repair", Value: map[string]any{"object_key": "order", "expected_schema_hash": "$instance.schema_hash", "payload": map[string]any{"name": "Customer", "type": "relation", "config": map[string]any{}}}, ExpectedErrorCodes: []string{"backend.metadata.relation_target_required"}},
		},
