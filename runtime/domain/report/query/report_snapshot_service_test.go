...
func (s *reportSnapshotMemoryStore) BeginReportSnapshot(_ context.Context, request reportcontract.ReportSnapshotBeginRequest) (reportcontract.ReportSnapshotClaim, error) {
	s.begin++
	if s.byKey == nil {
	...
	if snapshot, ok := s.byKey[key]; ok {
		disposition := reportcontract.ReportSnapshotClaimAcquired
		if snapshot.Status == "succeeded" {
			disposition = reportcontract.ReportSnapshotClaimReplay
		}
		return reportcontract.ReportSnapshotClaim{Snapshot: snapshot, Disposition: disposition}, nil
	}
...
func TestReportSnapshotRefreshSupportsObjectSQLThroughQueryPort(t *testing.T) {
	now := time.Date(2026, 7, 21, 12, 0, 0, 0, time.UTC)
	store := &reportSnapshotMemoryStore{}
	version := reportmodel.ReportSnapshotSourceVersion{Watermark: "object-sql-v1", SourceVersions: map[string]string{"sale": "1:hash"}}
	versions := &reportSnapshotVersionReader{versions: []reportmodel.ReportSnapshotSourceVersion{version, version}}
	executor := &objectSQLExecutorProbe{}
	report := reportmodel.ReportSchema{Key: "sales_sql", Materialization: &reportmodel.ReportMaterializationPolicy{MaximumLagSeconds: 60}, ObjectSQLV1: &reportmodel.ReportObjectSQLSchema{
		SQL: "SELECT SUM(s.amount) AS total FROM sale s LIMIT 10", SourceObjects: []string{"sale"},
	...
	service := NewReportDomainService(ReportDependencies{
		...
		Access:    objectSQLAccessProbe{objects: map[string]definitionmodel.ObjectSchema{"sale": {Key: "sale", Fields: []definitionmodel.FieldSchema{{Key: "amount", Type: "currency", Config: map[string]any{"precision": 19, "scale": 2}}}}}},
		ObjectSQL: executor, Snapshots: store, SnapshotSources: versions, Clock: func() time.Time { return now },
	})
	snapshot, err := service.RefreshSnapshot(t.Context(), report.Key, "window-sql", principalmodel.Principal{Principal: identitysdk.Principal{WorkspaceID: "workspace-a", UserID: "analyst"}})
	if err != nil || snapshot.Status != "succeeded" || snapshot.Summary.Rows[0].Measures["total"] != "1.00" || executor.calls != 1 || versions.calls != 2 {
		t.Fatalf("snapshot=%#v executor_calls=%d version_calls=%d err=%v", snapshot, executor.calls, versions.calls, err)
	}
}
