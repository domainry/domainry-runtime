...
type ReportDomainService struct {
	dependencies ReportDependencies
	snapshots    *reportsnapshot.ReportSnapshotDomainService
}
...
func NewReportDomainService(dependencies ReportDependencies) *ReportDomainService {
	...
	service := &ReportDomainService{dependencies: dependencies}
	service.snapshots = reportsnapshot.NewReportSnapshotDomainService(reportsnapshot.ReportSnapshotDependencies{
		Reports: dependencies.Reports, Store: dependencies.Snapshots,
		Query: service, Clock: dependencies.Clock,
	})
	return service
}

func (s *ReportDomainService) ReadReportSnapshotSourceVersion(ctx context.Context, report reportmodel.ReportSchema, principal principalmodel.Principal) (reportmodel.ReportSnapshotSourceVersion, error) {
	return s.ExportSourceVersion(ctx, report, principal)
}

// ExecuteReportSnapshot is the narrow query capability consumed by Snapshot.
// Snapshot owns refresh consistency and persistence state; Query owns how a
// report definition is evaluated.
func (s *ReportDomainService) ExecuteReportSnapshot(ctx context.Context, report reportmodel.ReportSchema, principal principalmodel.Principal) (reportmodel.ReportSummary, error) {
	...
	return s.executeReportDataset(ctx, report, plan, principal)
}
