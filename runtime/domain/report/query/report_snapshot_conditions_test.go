...
func (s *reportSnapshotFaultStore) BeginReportSnapshot(context.Context, reportcontract.ReportSnapshotBeginRequest) (reportcontract.ReportSnapshotClaim, error) {
	disposition := reportcontract.ReportSnapshotClaimRunning
	if s.beginExecute {
		disposition = reportcontract.ReportSnapshotClaimAcquired
	} else if s.beginSnapshot.Status == "succeeded" {
		disposition = reportcontract.ReportSnapshotClaimReplay
	}
	return reportcontract.ReportSnapshotClaim{Snapshot: s.beginSnapshot, Disposition: disposition}, s.beginErr
}
...
func TestRefreshSnapshotCoversLookupMaterializationIdempotencyDependencyPlanScopeAndBeginEdges(t *testing.T) {
	...
	dependencies = base
	dependencies.Snapshots = &reportSnapshotFaultStore{beginExecute: true}
...
func TestReportSnapshotScopeHashRejectsUnsupportedClaimAndVersionEqualityCoversBothOutcomes(t *testing.T) {
	if _, err := ReportAccessScopeHash(principalmodel.Principal{BusinessClaims: map[string]profilebindingmodel.ClaimValue{"bad": {Value: make(chan int)}}}); err == nil {
		t.Fatal("unsupported claim encoded")
	}
	version := reportmodel.ReportSnapshotSourceVersion{Watermark: "one"}
	if !ReportSourceVersionsEqual(version, version) || ReportSourceVersionsEqual(version, reportmodel.ReportSnapshotSourceVersion{Watermark: "two"}) {
...
func TestCommitSnapshotRefreshUnknownTerminalStatusIsNoop(t *testing.T) {
	...
	if err := service.CommitSnapshotRefresh(t.Context(), reportsnapshot.ReportSnapshotRefresh{TerminalStatus: "unknown"}); err != nil {
