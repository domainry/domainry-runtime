...
func ApplyWorkflowWithdrawalTx(ctx context.Context, store *RuntimeStore, tx ActionExecutionExecutor, workspaceID string, commit transactionmodel.WorkflowWithdrawalCommit) error {
	...
	statement, args, err := query.NewWorkspaceUpdateBuilder(store.SQLRenderer, "_workflow_process_instances", workspaceID).
		Set("status", p.Status).Set("current_node_ids_json", "[]").Set("variables_json", string(variables)).Set("result_json", string(result)).Set("updated_at", p.UpdatedAt).Set("completed_at", p.CompletedAt).
		Where(query.And(query.Equal("id", p.ID), query.Equal("workflow_key", p.WorkflowKey), query.Equal("initiator_id", commit.ActorID), query.Equal("status", commit.ExpectedStatus), query.Equal("updated_at", commit.ExpectedUpdatedAt))).Build()
	if err != nil {
		return err
	}
	...
	for _, item := range []struct {
		...
		if item.table == "_workflow_node_instances" {
			builder.Set("completed_at", p.CompletedAt)
		} else {
			builder.Set("updated_at", p.UpdatedAt)
			if item.table == "_workflow_executions" {
				builder.Set("node_id", "").Set("next_run_at", nil).Set("last_error", "").Set("lease_owner", "").Set("lease_expires_at", "")
			}
			if item.completed {
				builder.Set("completed_at", p.CompletedAt).Set("completed_by", commit.ActorID)
			}
		}
		active := query.In("status", item.statuses...)
		if item.table == "_workflow_executions" {
			active = query.Or(active, query.And(query.Equal("status", "failed"), query.NotEqual("next_run_at", "")))
		}
	...
	statement, args, err = query.NewWorkspaceInsertBuilder(store.SQLRenderer, "_workflow_process_events", workspaceID).
		...
		Values(commit.CommandID, p.ID, "", "", "process_cancelled", commit.ActorID, "workflow.event.process.cancelled", string(metadata), p.CompletedAt).Build()
	if err != nil {
		return err
	}
