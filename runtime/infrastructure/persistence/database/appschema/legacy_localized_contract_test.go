...
func TestManifestMetadataSeedsLocalizedTextAndPreservesUserOverrides(t *testing.T) {
	...
	defaultValues, err := store.ListLocalizedTexts(t.Context(), principalmodel.InstallationWorkspaceID, appschemamodel.LocalizedTextQuery{Locale: "zh-CN"})
	if err != nil {
	...
	values, err := store.ListLocalizedTexts(t.Context(), principalmodel.InstallationWorkspaceID, appschemamodel.LocalizedTextQuery{Locale: "en-US"})
	if err != nil {
	...
	if _, err := store.UpsertLocalizedText(t.Context(), principalmodel.InstallationWorkspaceID, appschemamodel.LocalizedTextUpsertRequest{
		EntityType: "field",
	...
	afterOverride, err := store.ListLocalizedTexts(t.Context(), principalmodel.InstallationWorkspaceID, appschemamodel.LocalizedTextQuery{EntityType: "field", EntityKey: "customer.status", Property: "name", Locale: "en-US"})
	if err != nil {
...
func assertLocalizedText(t *testing.T, values []appschemamodel.LocalizedText, entityType, entityKey, property, text string) {
	t.Helper()
