...
func (r ApplicationSchemaStore) PublishDefinition(ctx context.Context, scope principalmodel.SystemScope, resourceType, resourceKey string, req appschemamodel.ApplicationDefinitionUpsertRequest, audit auditmodel.AuditEvent) (appschemamodel.ApplicationDefinition, error) {
...
func (r ApplicationSchemaStore) publishDefinition(ctx context.Context, scope principalmodel.SystemScope, resourceType, resourceKey string, req appschemamodel.ApplicationDefinitionUpsertRequest, audit *auditmodel.AuditEvent) (appschemamodel.ApplicationDefinition, error) {
	...
	if len(req.Payload) == 0 {
		return appschemamodel.ApplicationDefinition{}, fmt.Errorf("metadata payload is required")
	}
	...
	if replay, found, replayErr := r.metadataDefinitionReplay(ctx, scope, resourceType, shape.Key, hash, req.ExpectedSchemaHash); replayErr != nil {
		return appschemamodel.ApplicationDefinition{}, replayErr
	} else if found {
		return replay, nil
	}
	...
	if err != nil {
		return appschemamodel.ApplicationDefinition{}, fmt.Errorf("begin metadata upsert: %w", err)
	}
	defer tx.Rollback()
	...
	if buildErr != nil {
		return appschemamodel.ApplicationDefinition{}, fmt.Errorf("build %s %s insert: %w", resourceType, shape.Key, buildErr)
	}
	if _, err := tx.ExecContext(ctx, query, args...); err != nil {
		...
		return appschemamodel.ApplicationDefinition{}, fmt.Errorf("insert %s %s: %w", resourceType, shape.Key, err)
	}
	...
	definition := appschemamodel.ApplicationDefinition{ResourceType: resourceType, ResourceKey: shape.Key, ObjectKey: shape.ObjectKey, Name: shape.Name, Payload: append([]byte(nil), raw...), SchemaVersion: version, SchemaHash: hash, SourceKind: sourceKind, SourceID: sourceID, CreatedAt: now, UpdatedAt: now}
	if audit != nil {
	...
	if err := r.refreshCatalogHashTx(ctx, tx, now); err != nil {
		return appschemamodel.ApplicationDefinition{}, fmt.Errorf("refresh active metadata revision: %w", err)
	}
	if err := tx.Commit(); err != nil {
		if replay, found, replayErr := r.metadataDefinitionReplay(ctx, scope, resourceType, shape.Key, hash, req.ExpectedSchemaHash); replayErr == nil && found {
			return replay, nil
		}
		return appschemamodel.ApplicationDefinition{}, fmt.Errorf("commit metadata upsert: %w", err)
	}
	return definition, nil
}

func (r ApplicationSchemaStore) insertDefinitionRefreshIntentTx(ctx context.Context, tx *sql.Tx, definition appschemamodel.ApplicationDefinition) error {
...
func (r ApplicationSchemaStore) CompleteDefinitionRefresh(ctx context.Context, scope principalmodel.SystemScope, resourceType, resourceKey, schemaHash, errorText string) error {
	if err := requireMetadataInstallationScope(scope); err != nil {
		return err
	}
...
func metadataDefinitionAuditValue(definition appschemamodel.ApplicationDefinition) map[string]any {
...
func (r ApplicationSchemaStore) DisableDefinition(ctx context.Context, scope principalmodel.SystemScope, resourceType, resourceKey string) error {
	if err := requireMetadataInstallationScope(scope); err != nil {
		return err
	}
	table, err := metadataDefinitionTable(resourceType)
...
func (r ApplicationSchemaStore) replaceDefinition(ctx context.Context, tx *sql.Tx, table, resourceType, resourceKey string, expectedHash *string) error {
	if expectedHash == nil {
	...
	if expected == "" {
		...
		return &appschemamodel.ApplicationDefinitionConflictError{ResourceType: resourceType, ResourceKey: resourceKey, ExpectedHash: expected, CurrentHash: current}
	}
...
func (r ApplicationSchemaStore) nextVersion(ctx context.Context, tx *sql.Tx, resourceType, resourceKey string) (string, error) {
...
func (r ApplicationSchemaStore) insertDefinitionVersion(ctx context.Context, tx *sql.Tx, resourceType, resourceKey, version, hash string, payload []byte, now string) error {
...
func (r ApplicationSchemaStore) ApplyDefinitionMutations(ctx context.Context, scope principalmodel.SystemScope, mutations []appschemamodel.ApplicationDefinitionMutation, audits []auditmodel.AuditEvent, publication *changeplanmodel.BusinessChangePlanPublication) ([]appschemamodel.ApplicationDefinition, error) {
	if err := requireMetadataInstallationScope(scope); err != nil {
		return nil, err
	}
	tx, err := r.database().BeginTx(ctx, recordMutationTxOptions())
	...
	defer tx.Rollback()
	definitions := make([]appschemamodel.ApplicationDefinition, 0, len(mutations))
...
func (r ApplicationSchemaStore) applyDefinitionUpsert(ctx context.Context, tx *sql.Tx, mutation appschemamodel.ApplicationDefinitionMutation) (appschemamodel.ApplicationDefinition, error) {
	...
	query, args, buildErr := ormbuilder.NewInsertBuilder(r.store.SQLRenderer, table).Columns(columns...).Values(values...).Build()
	if buildErr != nil {
		return appschemamodel.ApplicationDefinition{}, fmt.Errorf("build %s %s insert: %w", mutation.ResourceType, shape.Key, buildErr)
	}
	if _, err := tx.ExecContext(ctx, query, args...); err != nil {
		return appschemamodel.ApplicationDefinition{}, fmt.Errorf("insert %s %s: %w", mutation.ResourceType, shape.Key, err)
	}
	...
	return appschemamodel.ApplicationDefinition{ResourceType: mutation.ResourceType, ResourceKey: shape.Key, ObjectKey: shape.ObjectKey, Name: shape.Name, Payload: raw, SchemaVersion: version, SchemaHash: hash, SourceKind: sourceKind, SourceID: sourceID, CreatedAt: now, UpdatedAt: now}, nil
}

func (r ApplicationSchemaStore) applyDefinitionArchive(ctx context.Context, tx *sql.Tx, mutation appschemamodel.ApplicationDefinitionMutation) (appschemamodel.ApplicationDefinition, error) {
	table, err := metadataDefinitionTable(mutation.ResourceType)
	if err != nil {
		return appschemamodel.ApplicationDefinition{}, err
	}
	if mutation.Request.ExpectedSchemaHash == nil || strings.TrimSpace(*mutation.Request.ExpectedSchemaHash) == "" {
		return appschemamodel.ApplicationDefinition{}, &appschemamodel.ApplicationDefinitionConflictError{ResourceType: mutation.ResourceType, ResourceKey: mutation.ResourceKey}
	}
	...
	if buildErr != nil {
		return appschemamodel.ApplicationDefinition{}, fmt.Errorf("build %s %s archive: %w", mutation.ResourceType, mutation.ResourceKey, buildErr)
	}
	result, err := tx.ExecContext(ctx, query, args...)
	if err != nil {
		return appschemamodel.ApplicationDefinition{}, fmt.Errorf("archive %s %s: %w", mutation.ResourceType, mutation.ResourceKey, err)
	}
	affected, rowsErr := result.RowsAffected()
	if rowsErr != nil {
		return appschemamodel.ApplicationDefinition{}, fmt.Errorf("read archived %s %s rows: %w", mutation.ResourceType, mutation.ResourceKey, rowsErr)
	}
	if affected != 1 {
		return appschemamodel.ApplicationDefinition{}, &appschemamodel.ApplicationDefinitionConflictError{ResourceType: mutation.ResourceType, ResourceKey: mutation.ResourceKey, ExpectedHash: expected, CurrentHash: r.currentHash(ctx, tx, table, mutation.ResourceKey)}
	}
	return appschemamodel.ApplicationDefinition{ResourceType: mutation.ResourceType, ResourceKey: mutation.ResourceKey, SchemaHash: expected, DisabledAt: now, UpdatedAt: now}, nil
}

func (r ApplicationSchemaStore) insertChangeAudit(ctx context.Context, tx *sql.Tx, event auditmodel.AuditEvent) error {
	return auditmoduleimpl.AppendPreparedWithin(ctx, r.store.RuntimeRenderer(), runtimeauditmodule.NewTransaction(tx), event)
}

func (r ApplicationSchemaStore) syncLocalizedProjection(ctx context.Context, tx *sql.Tx, resourceType, resourceKey string, payload []byte, sourceID, now string) error {
	return r.syncMetadataLocalizedTextTx(ctx, tx, resourceType, resourceKey, payload, sourceID, now)
}

func (r ApplicationSchemaStore) currentHash(ctx context.Context, tx *sql.Tx, table, resourceKey string) string {
	var current string
...
func (r ApplicationSchemaStore) RollbackDefinition(ctx context.Context, scope principalmodel.SystemScope, resourceType, resourceKey string, request appschemamodel.ApplicationDefinitionRollbackRequest, audit auditmodel.AuditEvent) (appschemamodel.ApplicationDefinition, error) {
	if err := requireMetadataInstallationScope(scope); err != nil {
		return appschemamodel.ApplicationDefinition{}, err
	}
	table, err := metadataDefinitionTable(resourceType)
	...
	tx, err := r.database().BeginTx(ctx, recordMutationTxOptions())
	if err != nil {
		return appschemamodel.ApplicationDefinition{}, fmt.Errorf("begin rollback: %w", err)
	}
	defer tx.Rollback()
	...
	if buildErr != nil {
		return appschemamodel.ApplicationDefinition{}, fmt.Errorf("build rollback version lookup: %w", buildErr)
	}
	var payloadJSON, targetHash string
	if err := tx.QueryRowContext(ctx, query, args...).Scan(&payloadJSON, &targetHash); err == sql.ErrNoRows {
		return appschemamodel.ApplicationDefinition{}, fmt.Errorf("metadata.version.notFound: %s@%s", resourceKey, request.TargetVersion)
	} else if err != nil {
		return appschemamodel.ApplicationDefinition{}, fmt.Errorf("rollback read version: %w", err)
	}
	nextVersion, err := r.nextVersion(ctx, tx, resourceType, resourceKey)
	if err != nil {
		return appschemamodel.ApplicationDefinition{}, err
	}
	shape, err := metadataDefinitionShape(ctx, resourceType, resourceKey, appschemamodel.ApplicationDefinitionUpsertRequest{Payload: json.RawMessage(payloadJSON)})
	if err != nil {
		return appschemamodel.ApplicationDefinition{}, fmt.Errorf("rollback decode target: %w", err)
	}
	now := time.Now().UTC().Format(time.RFC3339)
	...
	if buildErr != nil {
		return appschemamodel.ApplicationDefinition{}, fmt.Errorf("build rollback update: %w", buildErr)
	}
	result, err := tx.ExecContext(ctx, update, updateArgs...)
	if err != nil {
		return appschemamodel.ApplicationDefinition{}, fmt.Errorf("rollback update: %w", err)
	}
	affected, rowsErr := result.RowsAffected()
	if rowsErr != nil {
		return appschemamodel.ApplicationDefinition{}, fmt.Errorf("read rollback update rows: %w", rowsErr)
	}
	if affected != 1 {
		return appschemamodel.ApplicationDefinition{}, &appschemamodel.ApplicationDefinitionConflictError{ResourceType: resourceType, ResourceKey: resourceKey, ExpectedHash: request.ExpectedSchemaHash, CurrentHash: r.currentHash(ctx, tx, table, resourceKey)}
	}
	...
	if err := r.refreshCatalogHashTx(ctx, tx, now); err != nil {
		return appschemamodel.ApplicationDefinition{}, fmt.Errorf("refresh metadata catalog hash during rollback: %w", err)
	}
	if err := tx.Commit(); err != nil {
		return appschemamodel.ApplicationDefinition{}, fmt.Errorf("commit rollback: %w", err)
	}
	definition, _, err := r.GetDefinition(ctx, scope, resourceType, resourceKey)
	return definition, err
}

func (r ApplicationSchemaStore) stageRollbackIntent(ctx context.Context, tx *sql.Tx, resourceType, resourceKey string, request appschemamodel.ApplicationDefinitionRollbackRequest, audit auditmodel.AuditEvent, now string) error {
