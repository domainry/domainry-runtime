...
func metadataObjectUpsertRequest() appschemamodel.ApplicationDefinitionUpsertRequest {
	return appschemamodel.ApplicationDefinitionUpsertRequest{Payload: []byte(`{"key":"account","name":"Account"}`)}
}
...
func TestPublishDefinitionValidationAndTransactionFailures(t *testing.T) {
	...
	if _, err := scriptedApplicationSchemaStore(t, &metadataSQLState{querySteps: []metadataSQLQueryStep{{err: errMetadataSQL}}}, base).publishDefinition(t.Context(), metadataInstallScope(), "object", "account", metadataObjectUpsertRequest(), nil); err == nil {
		t.Fatal("expected replay read error")
	}
	if _, err := scriptedApplicationSchemaStore(t, &metadataSQLState{querySteps: []metadataSQLQueryStep{metadataDefinitionMissingStep()}, beginErr: errMetadataSQL}, base).publishDefinition(t.Context(), metadataInstallScope(), "object", "account", metadataObjectUpsertRequest(), nil); err == nil {
