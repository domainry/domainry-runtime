...
func TestApplicationSchemaStorePublishesAndReadsEveryRecoveredDefinitionShape(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.resourceType, func(t *testing.T) {
			expectAbsent := ""
			published, err := publishDefinitionWithoutAuditForTest(t.Context(), repository, test.resourceType, test.resourceKey, appschemamodel.ApplicationDefinitionUpsertRequest{
