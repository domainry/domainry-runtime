...
func localizedTestText() appschemamodel.LocalizedText {
	return appschemamodel.LocalizedText{WorkspaceID: principalmodel.InstallationWorkspaceID, EntityType: "object", EntityKey: "account", Property: "name", Locale: "en-US", Text: "Account", SourceKind: "generated", SourceID: "template"}
}
...
func TestSyncLocalizedTextSQLBranches(t *testing.T) {
	...
	run := func(t *testing.T, state *metadataSQLState, text appschemamodel.LocalizedText) error {
		t.Helper()
	...
	if err := run(t, &metadataSQLState{}, appschemamodel.LocalizedText{}); err != nil {
		t.Fatal(err)
	}
	for _, invalid := range []appschemamodel.LocalizedText{
		{EntityType: "object"},
...
func TestLocalizedTextUpsertAndListSQLFailures(t *testing.T) {
	...
	t.Cleanup(func() { _ = baseDB.Close() })
	base := NewApplicationSchemaStore(baseDB)
	request := appschemamodel.LocalizedTextUpsertRequest{EntityType: "object", EntityKey: "account", Property: "name", Locale: "en-US", Text: "Account"}
	if _, err := base.UpsertLocalizedText(t.Context(), principalmodel.InstallationWorkspaceID, appschemamodel.LocalizedTextUpsertRequest{}); err == nil {
		t.Fatal("expected required fields error")
	}
	for _, invalid := range []appschemamodel.LocalizedTextUpsertRequest{
		{EntityType: "object"},
		{EntityType: "object", EntityKey: "account"},
		{EntityType: "object", EntityKey: "account", Property: "name"},
		{EntityType: "object", EntityKey: "account", Property: "name", Locale: "en-US"},
	...
	for _, step := range []metadataSQLQueryStep{
		...
		if _, err := repository.ListLocalizedTexts(t.Context(), principalmodel.InstallationWorkspaceID, appschemamodel.LocalizedTextQuery{EntityType: "object", EntityKey: "account", Property: "name", Locale: "en-US"}); err == nil {
