...
func (s ApplicationSchemaStore) UpsertApplicationDefinition(ctx context.Context, resourceType string, resourceKey string, req appschemamodel.ApplicationDefinitionUpsertRequest) (appschemamodel.ApplicationDefinition, error) {
	...
	schemaVersion, err := s.nextApplicationSchemaVersion(ctx, resourceType, shape.Key)
	if err != nil {
		return appschemamodel.ApplicationDefinition{}, err
	}
	...
	defer tx.Rollback()
	if err := s.replaceApplicationDefinitionVersion(ctx, tx, table, resourceType, shape.Key, req.ExpectedSchemaHash); err != nil {
		return appschemamodel.ApplicationDefinition{}, err
	}
	...
	if buildErr != nil {
		return appschemamodel.ApplicationDefinition{}, fmt.Errorf("build %s %s version insert: %w", resourceType, shape.Key, buildErr)
	}
	if _, err := tx.ExecContext(ctx, versionQuery, versionArgs...); err != nil {
		...
		return appschemamodel.ApplicationDefinition{}, fmt.Errorf("insert %s %s version: %w", resourceType, shape.Key, err)
	}
...
func (s ApplicationSchemaStore) replaceApplicationDefinitionVersion(ctx context.Context, tx *sql.Tx, table, resourceType, resourceKey string, expectedHash *string) error {
	if expectedHash == nil {
...
func (s ApplicationSchemaStore) nextApplicationSchemaVersion(ctx context.Context, resourceType string, resourceKey string) (string, error) {
...
// DisableApplicationDefinition soft-deletes a definition by setting disabled_at; no physical DROP is performed.
func (s ApplicationSchemaStore) DisableApplicationDefinition(ctx context.Context, resourceType string, resourceKey string) error {
	table, err := metadataDefinitionTable(resourceType)
	if err != nil {
		return err
	}
...
// ListApplicationDefinitions returns all active (non-disabled) definitions for the given resourceType.
...
// ListApplicationDefinitionVersions returns the version history for a definition.
func (s ApplicationSchemaStore) ListApplicationDefinitionVersions(ctx context.Context, resourceType string, resourceKey string) ([]appschemamodel.ApplicationDefinitionVersion, error) {
	...
	defer rows.Close()
	var out []appschemamodel.ApplicationDefinitionVersion
	for rows.Next() {
		var v appschemamodel.ApplicationDefinitionVersion
		var payloadJSON string
...
// RollbackApplicationDefinition restores a definition to a specific schema_version from its version history.
func (s ApplicationSchemaStore) RollbackApplicationDefinition(ctx context.Context, resourceType string, resourceKey string, request appschemamodel.ApplicationDefinitionRollbackRequest, audit auditmodel.AuditEvent) (appschemamodel.ApplicationDefinition, error) {
	table, err := metadataDefinitionTable(resourceType)
	if err != nil {
		return appschemamodel.ApplicationDefinition{}, err
	}
	...
	nextVersion, err := s.nextApplicationSchemaVersionTx(ctx, tx, resourceType, resourceKey)
	if err != nil {
		return appschemamodel.ApplicationDefinition{}, err
	}
	...
	if affected != 1 {
		return appschemamodel.ApplicationDefinition{}, &appschemamodel.ApplicationDefinitionConflictError{ResourceType: resourceType, ResourceKey: resourceKey, ExpectedHash: request.ExpectedSchemaHash, CurrentHash: s.currentMetadataHashTx(ctx, tx, table, resourceKey)}
	}
	if err := s.insertApplicationDefinitionVersionTx(ctx, tx, resourceType, resourceKey, nextVersion, targetHash, []byte(payloadJSON), now); err != nil {
		return appschemamodel.ApplicationDefinition{}, err
	}
	...
	d, _, err := s.GetApplicationDefinition(ctx, resourceType, resourceKey)
	return d, err
}

func (s ApplicationSchemaStore) stageMetadataRollbackIntentTx(ctx context.Context, tx *sql.Tx, resourceType, resourceKey string, request appschemamodel.ApplicationDefinitionRollbackRequest, audit auditmodel.AuditEvent, now string) error {
