...
func metadataTestStorageProfile(name string) appschemastorage.Profile {
	switch name {
	case "mysql":
...
func TestMetadataSnapshotRevisionBranches(t *testing.T) {
	...
	withoutDB := ApplicationSchemaStore{store: base.store}
...
func TestMetadataMigrationPlanRequestBranches(t *testing.T) {
	...
	if _, err := scriptedApplicationSchemaStore(t, state, repository).MigrationPlan(duringQuery, metadataInstallScope(), manifestmodel.ManifestSchema{Objects: []definitionmodel.ObjectSchema{{Key: "account"}}}); err == nil {
