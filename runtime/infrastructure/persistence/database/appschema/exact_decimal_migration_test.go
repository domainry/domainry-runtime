...
func TestSQLiteExactDecimalMigrationPreservesDataConstraintsAndEvidenceAcrossRestart(t *testing.T) {
	...
	if err := db.QueryRowContext(t.Context(), `SELECT COUNT(*), MAX(row_count), MAX(before_hash), MAX(after_hash) FROM _application_schema_exact_decimal_migration_receipts WHERE object_key='migration_parent'`).Scan(&evidenceRows, &rowCount, &beforeHash, &afterHash); err != nil {
		t.Fatal(err)
	}
	...
	if err := restarted.DB().QueryRowContext(t.Context(), `SELECT COUNT(*) FROM _application_schema_exact_decimal_migration_receipts WHERE object_key='migration_parent'`).Scan(&evidenceRows); err != nil || evidenceRows != 1 {
...
func TestSQLiteExactDecimalMigrationExternalFixture(t *testing.T) {
	...
	if os.Getenv("DOMAINRY_EXACT_DECIMAL_FIXTURE_EXPECT_BLOCKED") == "1" {
		...
		if err := store.DB().QueryRowContext(t.Context(), `SELECT COUNT(*) FROM _application_schema_exact_decimal_migration_receipts`).Scan(&evidenceRows); err != nil || evidenceRows != 0 {
	...
	if err := store.DB().QueryRowContext(t.Context(), `SELECT COUNT(*), COALESCE(SUM(row_count),0) FROM _application_schema_exact_decimal_migration_receipts`).Scan(&evidenceRows, &migratedRows); err != nil {
		t.Fatal(err)
	}
	if evidenceRows == 0 {
...
func TestSQLiteExactDecimalMigrationRollsBackOnScaleLoss(t *testing.T) {
	...
	var evidence int
	if err := db.QueryRowContext(t.Context(), `SELECT COUNT(*) FROM _application_schema_exact_decimal_migration_receipts WHERE object_key='migration_rollback'`).Scan(&evidence); err != nil || evidence != 0 {
