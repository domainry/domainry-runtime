...
func TestMySQLLargeEvidenceColumnNormalizationRepairsExistingTextColumns(t *testing.T) {
	...
	if err := ensureMySQLLargeEvidenceColumns(t.Context(), store); err != nil {
		t.Fatal(err)
	}
...
func TestLargeEvidenceColumnNormalizationIsMySQLOnly(t *testing.T) {
	...
	database := openSchemaScriptedDB(state)
	t.Cleanup(func() { _ = database.Close() })
	if err := ensureMySQLLargeEvidenceColumns(t.Context(), scriptedSchemaStore{db: database, driver: "sqlite"}); err != nil {
		t.Fatal(err)
	}
