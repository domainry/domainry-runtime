...
func EnsureWorkflowProcessSchema(ctx context.Context, s Store) error {
	...
	for column, definition := range map[string]string{
		...
		if err := s.EnsureRuntimeColumn(ctx, "workflow_tasks", column, definition); err != nil {
			return err
		}
	}
	if err := s.EnsureRuntimeColumn(ctx, "workflow_process_instances", "workflow_definition_version_id", text+" NOT NULL DEFAULT ''"); err != nil {
		return err
	}
	indexes := []struct {
		name, table string
		...
		{name: "uniq_workflow_process_workspace_id", table: "workflow_process_instances", columns: []string{"workspace_id", "id"}, unique: true},
		{name: "uniq_workflow_node_workspace_id", table: "workflow_node_instances", columns: []string{"workspace_id", "id"}, unique: true},
		{name: "uniq_workflow_task_workspace_id", table: "workflow_tasks", columns: []string{"workspace_id", "id"}, unique: true},
		{name: "uniq_workflow_event_workspace_id", table: "workflow_process_events", columns: []string{"workspace_id", "id"}, unique: true},
		{name: "idx_workflow_process_business_record", table: "workflow_process_instances", columns: []string{"workspace_id", "object_key", "record_id", "created_at"}},
		{name: "idx_workflow_definition_key", table: "workflow_definition_identities", columns: []string{"workflow_key"}, unique: true},
		{name: "idx_workflow_version_number", table: "workflow_definition_versions", columns: []string{"definition_id", "version_no"}, unique: true},
		{name: "idx_workflow_version_status", table: "workflow_definition_versions", columns: []string{"definition_id", "status", "updated_at"}},
		{name: "idx_workflow_publish_idempotency", table: "workflow_definition_versions", columns: []string{"definition_id", "publish_idempotency_key"}, unique: true},
		{name: "idx_workflow_process_status", table: "workflow_process_instances", columns: []string{"workspace_id", "status", "updated_at"}},
		{name: "idx_workflow_node_process", table: "workflow_node_instances", columns: []string{"workspace_id", "process_id", "node_id", "iteration"}, unique: true},
		{name: "idx_workflow_task_assignee", table: "workflow_tasks", columns: []string{"workspace_id", "assignee_user_id", "status", "created_at"}},
		{name: "idx_workflow_task_assignee_process", table: "workflow_tasks", columns: []string{"workspace_id", "assignee_user_id", "process_id"}},
		{name: "idx_workflow_task_process", table: "workflow_tasks", columns: []string{"workspace_id", "process_id", "node_id", "status"}},
		{name: "idx_workflow_event_process", table: "workflow_process_events", columns: []string{"workspace_id", "process_id", "created_at"}},
	}
