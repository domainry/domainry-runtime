...
func ensureEvidenceTables(ctx context.Context, s Store, tables map[string][]string, text string) error {
	...
	indexes := []struct {
		...
		{name: "uniq_runtime_operation_key", table: "_operations", columns: []string{"workspace_id", "system_purpose", "owner", "kind", "idempotency_key"}, unique: true},
		...
		{name: "idx_runtime_operation_expiry", table: "_operations", columns: []string{"workspace_id", "owner", "kind", "status", "expires_at"}},
