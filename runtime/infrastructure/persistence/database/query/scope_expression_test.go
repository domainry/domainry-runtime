...
func TestResolveScopeMembershipQueriesPermissionIDsBeforeRootFieldIN(t *testing.T) {
	...
	if !reflect.DeepEqual(lookupArgs, []any{"workspace-a", "member-1"}) {
...
func TestCandidateScopeMatchesConstrainsRelationLookupToUnpersistedCandidate(t *testing.T) {
	...
	if !reflect.DeepEqual(args, []any{"workspace-a", "warehouse-north", "order-north"}) {
...
func TestCandidateScopeMatchesUsesCandidateIDForReverseRelation(t *testing.T) {
	...
	matched, err := CandidateScopeMatches(fuzzQueryStore{}, "workspace-a", recordmodel.Record{ID: "order-new"}, expression, func(statement string, args ...any) (bool, error) {
		if !strings.Contains(statement, `permission_0."order_id" = $3`) || args[2] != "order-new" {
...
func TestResolveScopeMembershipFallsBackToSingleExistsAboveThreshold(t *testing.T) {
	...
	resolved, err := ResolveScopeMembership(fuzzQueryStore{}, "workspace-a", expression, 2, func(statement string, _ ...any) ([]string, error) {
		if !strings.HasSuffix(statement, "LIMIT 3") {
	...
	for _, fragment := range []string{`EXISTS (SELECT 1 FROM "member" permission_2`, `CROSS JOIN "card" permission_1`, `CROSS JOIN "account" permission_0`, `permission_0."id" = "ledger"."account_id"`} {
		if !strings.Contains(where, fragment) {
...
func TestResolveScopeMembershipUsesINAtOneThousand(t *testing.T) {
	...
	resolved, err := ResolveScopeMembership(fuzzQueryStore{}, "workspace-a", expression, 1000, func(statement string, _ ...any) ([]string, error) {
		if !strings.HasSuffix(statement, "LIMIT 1001") {
