...
func (r RecordStore) UpdateRecord(ctx context.Context, workspaceID string, object definitionmodel.ObjectSchema, record recordmodel.Record) error {
	...
	queryValue, args, buildErr := builder.Where(query.And(query.Equal("id", record.ID), s.SubjectRecordWriteAllowed(workspaceID, object.Key, record.ID, record.UpdateBy))).Build()
	if buildErr != nil {
		return buildErr
	}
	result, err := r.queryExecutor(ctx).ExecContext(ctx, queryValue, args...)
...
func (r RecordStore) UpdateRecordWhere(ctx context.Context, workspaceID string, object definitionmodel.ObjectSchema, record recordmodel.Record, conditions map[string]any) (bool, error) {
	...
	predicates := []query.Predicate{query.Equal("id", record.ID), s.SubjectRecordWriteAllowed(workspaceID, object.Key, record.ID, record.UpdateBy)}
...
func (r RecordStore) DeleteRecord(ctx context.Context, workspaceID string, object definitionmodel.ObjectSchema, recordID string) error {
	...
	defer tx.Rollback()
	queryValue, args, buildErr := query.NewWorkspaceDeleteBuilder(s.SQLRenderer, object.Key, workspaceID).Where(query.And(query.Equal("id", recordID), s.SubjectRecordWriteAllowed(workspaceID, object.Key, recordID, ""))).Build()
	if buildErr != nil {
		return buildErr
	}
