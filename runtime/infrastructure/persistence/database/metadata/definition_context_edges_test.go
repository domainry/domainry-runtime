...
func TestDefinitionDisableAndReplaceBranches(t *testing.T) {
	...
	if err := scriptedMetadataStore(t, &metadataSQLState{beginErr: errMetadataSQL}, base).DisableDefinition(t.Context(), metadataInstallScope(), "object", "account"); err == nil {
...
func TestDefinitionVersionAndAuditHelpers(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		var got string
		err := runMetadataTransaction(t, base, metadataSQLState{querySteps: []metadataSQLQueryStep{testCase.step}}, func(repository MetadataStore, tx *sql.Tx) error {
			var err error
	...
	valid := metadataDefinitionAuditValue(metadatamodel.MetadataDefinition{ResourceType: "object", Payload: []byte(`{"key":"account"}`)})
	invalid := metadataDefinitionAuditValue(metadatamodel.MetadataDefinition{ResourceType: "object", Payload: []byte(`{`)})
...
func TestDefinitionCatalogHashRefreshBranches(t *testing.T) {
	...
	for _, testCase := range tests {
		t.Run("direct "+testCase.name, func(t *testing.T) {
			repository := scriptedMetadataStore(t, &metadataSQLState{querySteps: metadataCatalogHashQuerySteps(testCase.first), execSteps: []metadataSQLExecStep{testCase.exec}}, base)
			if err := repository.refreshCatalogHash(t.Context()); (err != nil) != testCase.err {
				t.Fatalf("refresh err=%v", err)
			}
		})
		t.Run("transaction "+testCase.name, func(t *testing.T) {
			err := runMetadataTransaction(t, base, metadataSQLState{querySteps: metadataCatalogHashQuerySteps(testCase.first), execSteps: []metadataSQLExecStep{testCase.exec}}, func(repository MetadataStore, tx *sql.Tx) error {
