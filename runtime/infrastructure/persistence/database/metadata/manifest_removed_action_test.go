...
func TestSyncManifestDisablesRemovedGeneratedAutomationRules(t *testing.T) {
	...
	loaded, err := loadMetadataSlice[automationmodel.AutomationRuleSchema](t.Context(), store.MetadataStore, "automation_rule_definitions")
	if err != nil {
		t.Fatal(err)
	}
...
func TestDisableRemovedGeneratedActionsFailureAndSelectionBranches(t *testing.T) {
	...
	}, func(repository MetadataStore, tx *sql.Tx) error {
		return repository.disableRemovedGeneratedActions(t.Context(), tx, manifest, "now")
	}); err != nil {
		t.Fatal(err)
	}
...
func TestSyncManifestPropagatesRemovedActionAndCloseFailures(t *testing.T) {
	...
	disableRemovedGeneratedActionsForManifest = func(context.Context, MetadataStore, *sql.Tx, manifestmodel.ManifestSchema, string) error {
	...
	t.Cleanup(func() { _ = baseDB.Close() })
	base := NewMetadataStore(baseDB)
	err := runMetadataTransaction(t, base, metadataSQLState{querySteps: []metadataSQLQueryStep{{columns: []string{"resource_key"}}}}, func(repository MetadataStore, tx *sql.Tx) error {
