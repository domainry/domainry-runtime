...
func TestLegacyManifestLoadEveryReadStageFailure(t *testing.T) {
	...
	for stage := 0; stage <= 22; stage++ {
		...
		repository := scriptedMetadataStore(t, &metadataSQLState{querySteps: steps}, base)
	...
	if _, err := scriptedMetadataStore(t, &metadataSQLState{querySteps: steps}, base).LoadManifestMetadata(t.Context()); err == nil {
	...
	manifest, err := scriptedMetadataStore(t, &metadataSQLState{querySteps: steps}, base).LoadManifestMetadata(t.Context())
...
func TestLegacyManifestLoadPrimitiveFailures(t *testing.T) {
	...
	for _, step := range []metadataSQLQueryStep{
		...
		if _, err := scriptedMetadataStore(t, &metadataSQLState{querySteps: []metadataSQLQueryStep{step}}, base).loadMetadataCatalog(t.Context()); err == nil {
...
func TestLegacyMetadataMigrationPlanBranches(t *testing.T) {
	...
	cancel()
	if _, err := repository.MetadataMigrationPlan(cancelled, manifestmodel.ManifestSchema{Objects: []definitionmodel.ObjectSchema{{Key: "missing"}}}); err == nil {
		t.Fatal("expected cancelled migration plan")
	}
	steps, err := repository.MetadataMigrationPlan(t.Context(), manifestmodel.ManifestSchema{Objects: []definitionmodel.ObjectSchema{{Key: "missing", Fields: []definitionmodel.FieldSchema{{Key: "name", Type: "text"}}}}})
	...
	steps, err = repository.MetadataMigrationPlan(t.Context(), manifestmodel.ManifestSchema{Objects: []definitionmodel.ObjectSchema{{Key: "account", Fields: []definitionmodel.FieldSchema{{Key: ""}, {Key: "id"}, {Key: "amount", Type: "number"}}}}})
