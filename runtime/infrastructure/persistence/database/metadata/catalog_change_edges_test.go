...
func TestMetadataCatalogSQLBranches(t *testing.T) {
	...
	emptyQueries = metadataCatalogHashQuerySteps(metadataSQLQueryStep{columns: []string{"resource_key", "schema_hash"}})
	if err := scriptedMetadataStore(t, &metadataSQLState{querySteps: emptyQueries, execSteps: []metadataSQLExecStep{{err: errMetadataSQL}}}, base).refreshMetadataCatalogHash(t.Context()); err == nil {
...
func TestMetadataCatalogTransactionBranches(t *testing.T) {
	...
	t.Cleanup(func() { _ = baseDB.Close() })
	base := NewMetadataStore(baseDB)
	run := func(t *testing.T, state metadataSQLState, call func(MetadataStore, *sql.Tx) error) error {
		t.Helper()
	...
	for _, step := range []metadataSQLExecStep{{rows: 1}, {err: errMetadataSQL}} {
		err := run(t, metadataSQLState{execSteps: []metadataSQLExecStep{step}}, func(repository MetadataStore, tx *sql.Tx) error {
	...
	for _, testCase := range []struct {
		...
		err := run(t, metadataSQLState{execSteps: append([]metadataSQLExecStep(nil), testCase.steps...)}, func(repository MetadataStore, tx *sql.Tx) error {
	...
	for _, testCase := range []struct {
		...
		var got string
		err := run(t, metadataSQLState{querySteps: []metadataSQLQueryStep{testCase.step}}, func(repository MetadataStore, tx *sql.Tx) error {
			var err error
			got, err = repository.nextMetadataSchemaVersionTx(t.Context(), tx, "object", "account")
			return err
		})
		if got != testCase.want || (err != nil) != testCase.err {
	...
	for _, step := range []metadataSQLExecStep{{rows: 1}, {err: errMetadataSQL}} {
		err := run(t, metadataSQLState{execSteps: []metadataSQLExecStep{step}}, func(repository MetadataStore, tx *sql.Tx) error {
			return repository.insertMetadataDefinitionVersionTx(t.Context(), tx, "object", "account", "1", strings.Repeat("a", 64), []byte(`{}`), "now")
		})
		if (err != nil) != (step.err != nil) {
	...
	for _, testCase := range []struct {
		...
		var got string
		if err := run(t, metadataSQLState{querySteps: []metadataSQLQueryStep{testCase.step}}, func(repository MetadataStore, tx *sql.Tx) error {
