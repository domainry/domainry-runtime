...
func TestLoadManifestPropagatesEveryReadStageFailure(t *testing.T) {
	...
	if _, err := scriptedMetadataStore(t, &metadataSQLState{querySteps: steps}, base).LoadManifest(t.Context(), metadataInstallScope()); err == nil {
...
func TestManifestReadPrimitiveFailures(t *testing.T) {
	...
	for _, step := range []metadataSQLQueryStep{
		...
		if _, err := scriptedMetadataStore(t, &metadataSQLState{querySteps: []metadataSQLQueryStep{step}}, base).loadCatalog(t.Context()); err == nil {
...
func TestManifestDefinitionGetAndScanBranches(t *testing.T) {
	...
	if _, err := scanMetadataDefinition(metadataScannerFunc(func(...any) error { return sql.ErrConnDone }), "object"); !errors.Is(err, sql.ErrConnDone) {
...
func TestManifestDefinitionVersionReadAndSortBranches(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		repository := scriptedMetadataStore(t, &metadataSQLState{querySteps: []metadataSQLQueryStep{{columns: columns, rows: testCase.rows}}}, base)
...
func TestMetadataDefinitionReplayBranches(t *testing.T) {
	baseDB := openStoreForGeneratedListTest(t)
	t.Cleanup(func() { _ = baseDB.Close() })
	base := NewMetadataStore(baseDB)
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			repository := scriptedMetadataStore(t, &metadataSQLState{querySteps: append([]metadataSQLQueryStep(nil), testCase.steps...)}, base)
