...
func TestManifestStorageBootstrapRejectsCancelledContext(t *testing.T) {
	...
	if _, err := legacy.MetadataMigrationPlan(ctx, manifest); !errors.Is(err, context.Canceled) {
		t.Fatalf("MetadataMigrationPlan error = %v", err)
	}
}
...
func TestMetadataDefinitionCompatibilityAPIsRejectCancelledContext(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(t.Context())
	cancel()
	if _, err := legacy.ListMetadataDefinitions(ctx, "object", ""); !errors.Is(err, context.Canceled) {
		t.Fatalf("ListMetadataDefinitions error = %v", err)
	}
