...
func TestRelayRejectsLateCompletionFromExpiredLease(t *testing.T) {
	...
	var remote string
	var fencing int64
	if err := publication.runtime.DB().QueryRowContext(t.Context(), "SELECT remote_event_id,fencing_token FROM notification_publication_outbox WHERE id=?", intent.ID).Scan(&remote, &fencing); err != nil {
		t.Fatal(err)
	}
...
func TestRelayRecoversExpiredLeaseAfterProcessRestart(t *testing.T) {
	...
	var fencing int64
	if err := restarted.DB().QueryRowContext(t.Context(), "SELECT status,lease_owner,remote_event_id,fencing_token FROM notification_publication_outbox WHERE id=?", intent.ID).Scan(&status, &owner, &remote, &fencing); err != nil {
		t.Fatal(err)
	}
...
func TestRelayDeadLettersAfterBoundedUnknownOutcomeRetries(t *testing.T) {
	...
	if _, err := store.DB().ExecContext(t.Context(), "UPDATE notification_publication_outbox SET attempt_count=?,next_attempt_at='' WHERE id=?", publicationMaxAttempt-1, intent.ID); err != nil {
		t.Fatal(err)
	}
	clock := &relayClock{now: time.Date(2026, 8, 28, 1, 0, 0, 0, time.UTC)}
	...
	var status, code, terminal string
	var attempts int
	if err := store.DB().QueryRowContext(t.Context(), "SELECT status,last_error_code,terminal_at,attempt_count FROM notification_publication_outbox WHERE id=?", intent.ID).Scan(&status, &code, &terminal, &attempts); err != nil {
		t.Fatal(err)
	}
