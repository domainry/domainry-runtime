...
func (r *Relay) ProcessDue(ctx context.Context, limit int) (int, error) {
	...
	query, args, buildErr := ormbuilder.NewSelectBuilder(s.SQLRenderer, "_publication_outbox").Columns("workspace_id", "id").Where(due).OrderBy(ormbuilder.Ascending("created_at"), ormbuilder.Ascending("id")).Limit(limit).Build()
	if buildErr != nil {
...
func (r *Relay) claim(ctx context.Context, locator workerplatform.DurableTaskLocator) (publication, bool, error) {
	...
	query, args, buildErr := ormbuilder.NewWorkspaceUpdateBuilder(s.SQLRenderer, "_publication_outbox", locator.WorkspaceID).Set("status", "sending").Set("last_attempt_at", nowText).Set("lease_owner", r.workerID).Set("lease_expires_at", expires).SetExpression("fencing_token", ormbuilder.Add(ormbuilder.Column("fencing_token"), ormbuilder.Value(1))).Set("updated_at", nowText).Where(ormbuilder.And(ormbuilder.Equal("publication_type", "notification.saas"), ormbuilder.Equal("id", locator.TaskID), publicationDuePredicate(nowText))).Build()
	if buildErr != nil {
...
func (r *Relay) transition(ctx context.Context, value publication, status, next, code, remoteEventID, message string, terminal bool) error {
	...
	query, args, buildErr := ormbuilder.NewWorkspaceUpdateBuilder(s.SQLRenderer, "_publication_outbox", value.WorkspaceID).Set("status", status).SetExpression("attempt_count", ormbuilder.Add(ormbuilder.Column("attempt_count"), ormbuilder.Value(1))).Set("next_attempt_at", next).Set("lease_owner", "").Set("lease_expires_at", "").Set("remote_event_id", remoteEventID).Set("last_error_code", code).Set("last_error", message).Set("terminal_at", terminalAt).Set("updated_at", now).Where(ormbuilder.And(ormbuilder.Equal("publication_type", "notification.saas"), ormbuilder.Equal("id", value.ID), ormbuilder.Equal("status", "sending"), ormbuilder.Equal("lease_owner", value.LeaseOwner), ormbuilder.Equal("fencing_token", value.FencingToken))).Build()
	if buildErr != nil {
		return buildErr
	}
	result, err := s.DB().ExecContext(ctx, query, args...)
