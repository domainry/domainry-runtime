...
func TestNotificationDeliveryReservationCleanupIsWorkspaceScoped(t *testing.T) {
	store := openLifecycleStore(t)
	now := time.Date(2026, 7, 19, 12, 0, 0, 0, time.UTC)
	old := lifecycleTime(now.Add(-3 * 365 * 24 * time.Hour))
	for _, item := range []struct{ id, workspace string }{{"reservation-a", "workspace-a"}, {"reservation-b", "workspace-b"}} {
		if _, err := store.DB().ExecContext(t.Context(), "INSERT INTO notification_delivery_reservations (id, workspace_id, recipient_key, template_key, channel, dedupe_key, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)", item.id, item.workspace, "user-1", "notice", "email", item.id, old); err != nil {
			t.Fatal(err)
		}
	}
	executor := ownerExecutorForTest(t, DefaultOwnerExecutors(store), "notification")
	policy := lifecyclemodel.PolicyVersion{Policy: lifecyclemodel.RetentionPolicy{Key: "notification.history.v1", Version: "1", Owner: "notification", DefaultRetention: 2 * 365 * 24 * time.Hour}}
	result, err := executor.ProcessBatch(t.Context(), lifecyclemodel.CleanupJob{ID: "cleanup-notification", WorkspaceID: "workspace-a", Operation: lifecyclemodel.OperationPurge, UpdatedAt: now}, policy, nil, 10)
	if err != nil || result.Archived != 1 || result.Purged != 1 {
		t.Fatalf("result=%#v err=%v", result, err)
	}
	for workspace, want := range map[string]int{"workspace-a": 0, "workspace-b": 1} {
		var count int
		if err := store.DB().QueryRowContext(t.Context(), "SELECT COUNT(*) FROM notification_delivery_reservations WHERE workspace_id = ?", workspace).Scan(&count); err != nil || count != want {
			t.Fatalf("workspace=%s count=%d want=%d err=%v", workspace, count, want, err)
		}
	}
}

func TestNotificationHistoryCleanupPurgesOnlyTerminalEventsAndChannelPlans(t *testing.T) {
	store := openLifecycleStore(t)
	now := time.Date(2026, 7, 19, 12, 0, 0, 0, time.UTC)
	old := lifecycleTime(now.Add(-181 * 24 * time.Hour))
	for _, value := range []struct{ id, status string }{{"event-terminal", "materialized"}, {"event-active", "processing"}} {
		_, err := store.DB().ExecContext(t.Context(), "INSERT INTO notification_events (id, workspace_id, source, source_event_id, status, payload_json, attempt_count, next_attempt_at, last_error_code, lease_owner, lease_expires_at, fencing_token, occurred_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", value.id, "workspace-a", "workflow", value.id, value.status, `{}`, 0, "", "", "", "", 0, old, old, old)
		if err != nil {
			t.Fatal(err)
		}
	}
	for _, value := range []struct{ id, status string }{{"plan-terminal", "failed"}, {"plan-active", "queued"}} {
		_, err := store.DB().ExecContext(t.Context(), "INSERT INTO notification_channel_plans (id, workspace_id, event_id, channel, status, payload_json, attempt_count, next_attempt_at, last_error_code, outbox_message_id, lease_owner, lease_expires_at, fencing_token, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", value.id, "workspace-a", "event-"+value.id, "email", value.status, `{}`, 0, "", "", "", "", "", 0, old, old)
		if err != nil {
			t.Fatal(err)
		}
	}
	if _, err := store.DB().ExecContext(t.Context(), "INSERT INTO notification_event_failures (id, workspace_id, event_id, event_type, source, source_event_id, stage, error_code, attempt, disposition, retryable, next_attempt_at, fencing_token, occurred_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", "failure-terminal", "workspace-a", "event-terminal", "workflow.task.assigned", "workflow", "event-terminal", "materialization", "backend.notification.inbox_materialization_failed", 1, "dead_letter", 0, "", 1, old); err != nil {
		t.Fatal(err)
	}
	executor := ownerExecutorForTest(t, DefaultOwnerExecutors(store), "notification")
	policy := lifecyclemodel.PolicyVersion{Policy: lifecyclemodel.RetentionPolicy{Key: "notification.history.v1", Version: "1", Owner: "notification", DefaultRetention: 180 * 24 * time.Hour}}
	result, err := executor.ProcessBatch(t.Context(), lifecyclemodel.CleanupJob{ID: "cleanup-notification-history", WorkspaceID: "workspace-a", Operation: lifecyclemodel.OperationPurge, UpdatedAt: now}, policy, nil, 20)
	if err != nil || result.Purged != 3 {
		t.Fatalf("result=%#v err=%v", result, err)
	}
	for table, want := range map[string]int{"notification_events": 1, "notification_channel_plans": 1, "notification_event_failures": 0} {
		var count int
