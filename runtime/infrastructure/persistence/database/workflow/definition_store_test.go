...
func TestWorkflowDefinitionStoreLifecycleAndCancellation(t *testing.T) {
	...
	definition := workflowmodel.WorkflowDefinition{ID: "definition-1", Key: "expense", Name: "Expense", Enabled: true, CurrentDraftVersionID: "version-1", CreatedAt: workflowTestTimeV1, UpdatedAt: workflowTestTimeV1}
	draft := workflowmodel.WorkflowDefinitionVersion{ID: "version-1", DefinitionID: definition.ID, Version: 1, Status: workflowmodel.WorkflowVersionDraft, Revision: 1, Workflow: definitionmodel.WorkflowSchema{Key: definition.Key, Name: definition.Name, Graph: &definitionmodel.WorkflowGraphSchema{Version: 2}}, ValidationReport: workflowmodel.WorkflowValidation{Valid: true}, CreatedBy: "admin", CreatedAt: workflowTestTimeV1, UpdatedAt: workflowTestTimeV1}
	...
	draft.Workflow.Name, draft.UpdatedAt = "Expense updated", workflowTestTimeV2
	if updated, err := repository.UpdateDraft(t.Context(), draft, 1); err != nil || !updated {
		t.Fatalf("update draft=%v err=%v", updated, err)
	}
	draft.Revision, draft.ContentHash, draft.PublishedBy, draft.PublishedAt = 2, "hash-1", "admin", workflowTestTimeV3
	definition.UpdatedAt = workflowTestTimeV3
	...
	if updated, err := repository.SetDefinitionEnabled(t.Context(), definition.ID, false, workflowTestTimeV4); err != nil || !updated {
	...
	second.CreatedAt, second.UpdatedAt = workflowTestTimeV5, workflowTestTimeV5
	...
	if archived, err := repository.ArchiveVersion(t.Context(), definition.ID, draft.ID, workflowTestTimeV6); err != nil || archived {
