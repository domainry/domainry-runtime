...
func TestAutomationInstructionLeaseContractAcrossDialects(t *testing.T) {
	...
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			...
			defer store.DB().ExecContext(t.Context(), "DELETE FROM "+store.Identifier("automation_instruction_executions")+" WHERE "+store.Identifier("workspace_id")+" = "+store.Placeholder(1), workspace)
