...
func TestApplyMigrationFileSQLFailures(t *testing.T) {
	...
	tests := []struct {
		...
		{"postgres search path", databaseSQLState{execSteps: []databaseSQLExecStep{{rows: 1}, {err: errDatabaseSQL}}}, func(store *RuntimeStore) { store.engine = postgres.NewEngine() }, "search path"},
