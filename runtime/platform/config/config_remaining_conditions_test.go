...
func TestConfigValidateEveryShortCircuitOperand(t *testing.T) {
	...
	tests := []struct {
		...
		{"batch queue zero", func(c *Config) { c.CapacityBatchJobQueueLimit = 0 }},
		{"batch queue high", func(c *Config) { c.CapacityBatchJobQueueLimit = 1_000_001 }},
		{"batch workspace queue zero", func(c *Config) { c.CapacityBatchJobWorkspaceQueueLimit = 0 }},
