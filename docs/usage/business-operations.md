...
- Recording a local settlement intent atomically, then letting Integration perform the irreversible provider call after commit.
...
| Settle an invoice and charge a payment Provider. | Atomic local Operation plus governed post-commit Integration invocation | Commit the settlement intent and stable provider idempotency identity locally; dispatch the typed Provider operation after commit and reconcile its receipt. | Holding the database transaction open during the network call or retrying with a fresh provider identity. |
  ...
  "operations": [
    {
      ...
      "owner": "business",
      "scope": "record",
      "input": [{"key": "expected_version", "name": "Expected version", "type": "integer", "required": true}],
      "output": [{"key": "status", "name": "Status", "type": "select", "options": ["active"]}],
      "errors": [
        {"code": "account.not_pending", "message": "Only a pending account can be activated"},
        {"code": "account.activation_conflict", "message": "Account changed before activation"}
      ],
      "capabilities": [
...
The Handler reads the routed record under lock, compares `expected_version`, verifies `status == pending`, performs the conditional update, records actor/time, and returns errors unchanged so Runtime rolls back the complete command. Use the Operation invocation identity as the idempotency key: a replay returns the prior result, while a stale version returns `account.activation_conflict`; neither repeats side effects.
...
Waiting, approval tasks, recurrence, analytical reports, notification policy, and provider protocol belong to their own owners. If the required typed capability is absent, implement the smallest source-owned contract extension and its Deck schema, semantic validation, lowering/codegen, generated authority, and acceptance tests; do not weaken or abandon the business command.
