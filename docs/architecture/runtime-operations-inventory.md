...
| Integration | `POST /integrations/events/{eventID}/retry` | authenticated; workspace scope | required caller key plus owner record semantics | event status evidence and terminal shared receipt | wrapped by `integration.event.retry` |
| Integration | `POST /integrations/events/{eventID}/replay` | authenticated; workspace scope | required caller key plus owner replay semantics | correlated event evidence and terminal shared receipt | wrapped by `integration.event.replay`; unified DLQ path also rechecks current workspace/config/secret/handler/mapping readiness |
| Integration | `POST /integrations/outbox/{messageID}/retry` | authenticated; workspace scope | required caller key plus owner record semantics | outbox delivery evidence and terminal shared receipt | wrapped by `integration.outbox.retry`; uncertain/partial outcomes remain quarantined for reconciliation and are never blind-resent |
| Integration | `ReconcileMissingIntegrationReceipts` worker use case | explicit system scope | natural stale invocation identity | reconciliation result | no operator receipt/status URL |
