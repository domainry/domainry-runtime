...
| Scheduler command | `SimulateJob`, `RunJob`, `RetryRun`, `CancelRun`, `ResolveDeadLetter`, `ClaimRun`, `HeartbeatRun`, `AppendRunEvent`, `FinishRun`, `ProcessClaimedRun` | T19-T20 | `application/scheduler/scheduler_operations.go`, `scheduler_persistence.go`, `scheduler_runs.go`, `scheduler_runtime.go` | Caller commands and worker-owned run lifecycle mutations are inventoried separately; `SimulateJob` persists a run and event despite its name, while status/due-definition reads are excluded. |
| Integration event | `RecordIntegrationEvent`, `ReceiveIntegrationWebhookForWorkspace`, `ProcessDueIntegrationEvents` | T21-T22 | `application/integration/integration_application_events.go`, `integration_application_inbound_webhooks.go`, `integration_application_workers.go` | Acceptance and worker execution are separate consistency boundaries. Acceptance now persists the event and durable mapping intent atomically; target-domain execution remains recoverable asynchronous work. |
...
| T19 | `SimulateJob`, `RunJob`, `RetryRun`, `CancelRun`, `ResolveDeadLetter` | definition/run/dead-letter records, receipt | run state, run event, retry/cancel/dead-letter state, receipt | caller idempotency key except simulation; run/dead-letter status checks | scheduler operation evidence is stored as run events; separate Audit varies by command | workflow execution may be created by run processing | none observed | processing is invoked after run claim/command and can continue asynchronously |
| T20 | `ClaimRun`, `HeartbeatRun`, `AppendRunEvent`, `FinishRun`, `ProcessClaimedRun` | definition/run plus lease owner/token and workflow executions | lease/run state, run events, retry/dead-letter schedule, workflow execution state | lease owner, expiry and fencing/conditional update | run-event history is the worker evidence | process execution updates/creates workflow facts | none observed | workflow execution occurs while the claimed run is owned; crash recovery depends on durable lease/state |
| T21 | `RecordIntegrationEvent`, `ReceiveIntegrationWebhookForWorkspace` | connector/handler registry, event mapping schema, external event identity | integration event and `integration_event_mapping_intents` row | unique external event identity and unique event intent | acceptance Audit follows the atomic commit | durable mapping intent records mapping key/target | provider delivery receipt uses the separate outbound outbox boundary | identity writeback and delivery-receipt handling remain follow-up operations after acceptance |
| T22 | `ProcessDueIntegrationEvents` | due event, lease/fencing token, mapping intent/schema and target-domain state | event lease/status/retry plus target Workflow/Action/Record facts and execution evidence | event lease owner and fencing token | worker and mapping Audits are follow-up evidence | consumes the accepted mapping route | target action may enqueue integration outbox | target-domain execution cannot share one transaction with every supported owner and requires reconciliation/compensation |
...
| T19 Scheduler caller commands | `atomic_required` | `eventually_consistent` | command receipt + run/event/retry/dead-letter state are atomic; execution follows durable run state |
...
The following neighboring use cases are explicitly `read_only` and therefore remain outside the mutation table: Record export and Import preview; Action, Workflow, Metadata and ChangePlan validate/simulate/preview methods; Scheduler definition preview/validation and status/due-definition queries; Integration event/outbox list and get queries; list/get/diff/status/catalog methods across all eleven owners. `SimulateJob` is deliberately not in this exclusion because it persists a run and event. A future write introduced under another read-only name must first move into the mutation inventory.
...
| MW08 | `ChangePlan.ApplyIdempotent` / `Apply` | execute delegated plan items -> publish draft -> insert plan Audit -> complete receipt | `application/changeplan/changeplan_business_change_plan_apply_export.go` | plan items can partially apply and the draft/receipt may disagree with actual applied scope |
...
Resolved high-risk migrations are removed from the unresolved table. Scheduler MW12-MW15 now prepare the command state and run-event records in Application, commit them through one `CommitRecordMutationBatch`, and only then execute Workflow continuation or optional telemetry. Scheduler finish/dead-letter state already uses the same batch boundary. A failed event insert therefore rolls back the run/dead-letter transition instead of leaving state without evidence.
...
Identity MW09-MW10 are resolved through `IdentityAtomicMutationRepository`. Role permission/data-scope/field-permission assignment rows and the denormalized role projection commit in one database transaction. Recursive menu removal first computes the complete child-first set, then removes assignments and soft-deletes the entire set atomically. Domain service methods fail closed when the atomic adapter is unavailable. The remaining Plane-owned unresolved table contains only deliberately resumable or compensatable batch/cross-boundary flows (MW01-MW03 and MW08), each already classified as `eventually_consistent` and/or `compensatable` with durable progress/reconciliation evidence.

## Transaction callback side-effect audit

Audit date: 2026-07-19.

| Audit surface | Result | Evidence/decision |
| --- | --- | --- |
...
The shared SQL Unit of Work contract runs the same four-window failure matrix for every P2 critical chain family: Record mutation, Workflow task decision, Action execution, Scheduler run state, Integration event acceptance, Metadata publication, and cross-boundary durable intent.
...
- Bulk, Import, scheduler workers, and due-publication processing are resumable/batch shapes and cannot inherit single-record transaction assumptions.
