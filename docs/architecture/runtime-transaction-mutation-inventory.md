...
| Scheduler command | Runtime definition preview only; run/retry/cancel/dead-letter commands are external | T19-T20 | `domainry-scheduler` / `domainry-scheduler-sdk`; Runtime callback wiring is `bootstrap/composition/scheduler_sdk_module_host_wiring.go` | Scheduler mutation profiles remain inventoried as external owner contracts so Runtime cannot silently recreate a second run ledger. |
...
| T19 Scheduler caller commands | `atomic_required` | `eventually_consistent` | enforced by source-owned Scheduler; Runtime exposes only callback dispatch and read-only definition preview |
...
The following neighboring use cases are explicitly `read_only` and therefore remain outside the mutation table: Record export and Import preview; Action, Workflow, Metadata and ChangePlan validate/simulate/preview methods; Scheduler definition preview/validation; Integration event/outbox list and get queries; list/get/diff/status/catalog methods across all eleven owners. Scheduler run, event, retry, cancel, dead-letter, claim, heartbeat and finish state is source-owned by `domainry-scheduler`; Runtime must not recreate those mutations. A future write introduced under another read-only name must first move into the mutation inventory.
...
Resolved high-risk migrations are removed from the unresolved table. Scheduler run lifecycle consistency is enforced by the source-owned Scheduler module in both Module and SaaS topologies; Runtime owns only callback dispatch and owner-specific timers such as `record_timer`.
