...
| T09 | `UpsertMetadataDefinition`, `DisableMetadataDefinition` | current definition, validation/reference graph, current manifest/snapshot | definition/current version/history, Audit and active catalog revision | repository expected/current revision semantics; publication uses one serializable commit | publication Audit is stored in the same commit; disable Audit path remains separate | none; there is no asynchronous refresh consumer | none observed | Runtime metadata is refreshed synchronously after commit and a refresh failure is returned to the caller; the next reload reads the durable current revision |
...
| T19 | Scheduler submit/retry/cancel/dead-letter commands | Scheduler-owned definition/run/dead-letter facts | Scheduler-owned run retry/cancel/dead-letter fields and command receipt | Scheduler-owned idempotency and status transition guards | terminal run fields plus resolution actor/time/operation evidence | callback may start Runtime-owned downstream work | Scheduler-owned delivery | implemented and verified in `domainry-scheduler`; Runtime owns no copy of these writes |
| T20 | `ClaimRun`, `HeartbeatRun`, `FinishRun`, `ProcessClaimedRun` | definition/run plus lease owner/token and workflow executions | lease/run state, retry/dead-letter schedule, workflow execution state | lease owner, expiry and fencing/conditional update | fenced run status, attempt, failure and terminal timestamps are the worker evidence | process execution updates/creates workflow facts | none observed | source-owned Scheduler worker lifecycle; Runtime owns no copy of these writes |
| T21 | Integration `AcceptWebhook` | Connector verifier, shared event-mapping Definition, external event identity | Integration-owned event row, including the synchronous Runtime execution receipt when mapped | unique external event identity and webhook replay evidence | owner-local acceptance and execution evidence on the event | none; mapping selection is current event execution state, not pending dispatch | SDK `TriggerSink` may execute Runtime Action/Workflow after event acceptance | implemented in `domainry-integration`; Runtime owns no copy of these writes, and cross-owner execution is compensatable/eventual |
| T22 | Integration `ProcessDueEvents` | due event, lease/fencing token and shared event-mapping Definition | Integration event lease/status/retry plus mapping key, target type and Runtime execution receipt on the same event | Integration lease owner and fencing token; Runtime target idempotency key | Integration-owned execution receipt | none | SDK `TriggerSink` executes Runtime Action/Workflow and may create Runtime publication handoff | cross-owner execution is compensatable/eventual and cannot share one SQL transaction |
