...
| T21 | `RecordIntegrationEvent`, `ReceiveIntegrationWebhookForWorkspace` | connector/handler registry, event mapping schema, external event identity | integration event and `_integration_event_mapping_intents` row | unique external event identity and unique event intent | acceptance Audit follows the atomic commit | durable mapping intent records mapping key/target | provider delivery receipt uses the separate outbound outbox boundary | identity writeback and delivery-receipt handling remain follow-up operations after acceptance |
