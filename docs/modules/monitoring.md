...
产品 HTTP `GET /monitoring/metrics` 由 Monitoring Binding 的 `modulehttp.Adapter` 声明和实现。Runtime 只负责通用 Adapter 挂载、宿主身份鉴权、监听器隔离及从 Adapter 声明生成 OpenAPI；Runtime 不保留 Monitoring handler、静态 endpoint inventory 或静态 OpenAPI 路径。
...
- HTTP：`domainry-monitoring/internal/transport/http/module/adapter.go`
