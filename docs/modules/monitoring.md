...
产品 HTTP `GET /monitoring/metrics` 由 Monitoring Binding 的 `modulehttp.Adapter` 声明和实现。Runtime 只负责通用 Adapter 挂载、宿主身份鉴权和监听器隔离；Monitoring 当前 SDK 源码和契约测试定义精确接口，Runtime 不保留 Monitoring handler 或第二套路由描述。
