...
`GET /operations/scheduler/state` 通过 SDK `Binding.Runs` 与 `Binding.DeadLetters` 读取 owner 状态；DTO 只存在于 HTTP transport。Runtime Application 不再声明 run/dead-letter 状态模型，也不落一份镜像 Record。
