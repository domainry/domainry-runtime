...
Module migration 必须通过 `ApplyOwnedMigrations`。SaaS publication 交接状态 `notification_publication_outbox` 已登记为 Runtime workspace-scoped handoff；Notification 自有表不属于 Runtime schema 清单。
