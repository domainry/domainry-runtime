...
`drill` profile must produce validated recovery evidence.
...
| `drill` | Docker, `jq`, `sqlite3` | per-engine restore JSON with actual RPO/RTO plus key-rotation and retention logs | backup/restore and post-restore reconciliation are executable, not prose-only |
